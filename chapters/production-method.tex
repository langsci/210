...
However, this distinct \is{\isi{distinctness}}ion was largely based on experts' judgements and evaluations, and especially external \isi{stereotype}s may well ``become increasingly divorced from the forms which are actually used in speech'' \parencite[180]{labov1972}.
This part of the book will therefore try to corroborate the alleged \isi{salience} of the variables `from the inside', as it were, and also to go beyond the simple binary distinct \is{\isi{distinctness}}ion of salient \is{salience}/non-salient \is{salience} by ordering our four variables more precisely in relation to each other on the social \isi{salience} scale.
...
The interviews consisted of a free speech section where subjects were asked a number of questions about the area of the city they grew up in, \isi{change}s in the city, football and other sports, Liverpool's \isi{image} in the UK and the rivalry with Manchester.\footnote{Although this rivalry has historical reasons (cf. Chapter \ref{ch.hist}), it is today dominated by the rivalry between the football clubs from Liverpool and Manchester in many people's minds. This does not, however, diminish its potential for bringing up questions of \isi{identity} and local pride in the slightest. Indeed, as \textcite[97]{beal2010} remarks, ``[t]he football derby (\dots) is one of the clearest manifestations of local \isi{identity} and rivalry in Britain today''}
...
In graphs showing register differences, the data gathered during the accent imitation \is{accent perform \is{accent performance}ance} task will be situated towards the informal end of the style spectrum.
I am aware \is{\isi{awareness}} of the fact that the imitation \is{accent perform \is{accent performance}ance} style is almost certainly one where subjects are likely to pay \emph{more} than average attention to their speech.
A reviewer quite correctly points out that \isi{accent perform \is{accent performance}ance} probably qualifies as a \enquote{frozen, ritualistic} style \parencite{labov1972} that, in terms of attention, should rather be placed towards the more formal end of the style continuum.
...
A linear increase of local variants can be expected from word list through reading and free speech style to accent imitation \is{accent perform \is{accent performance}ance}, so it seems to me the placement of the latter towards the \enquote*{informal} end of the style spectrum is justified in that respect.
Purely for reasons of convenience, accent imitation \is{accent perform \is{accent performance}ance} will occasionally be referred to as the \enquote*{most informal} speech style, simply because it should be the most \enquote*{vernacular} register, \emph{not} because I believe subjects paid no attention to their speech.

Finally, subjects were asked a number of questions concerning Scouse, notably whether they thought the accent had \isi{change}d in their life time and what features they considered most typical.
Analysis of these statements can only be qualitative in nature and should be considered an impressionistic snapshot rather than anything close to a representative picture of the relevant groups' explicit linguistic knowledge. Usually, the interviews lasted between 50 and 60 minutes (40--45 minutes of free speech and 10--15 minutes of reading/accent imitation \is{accent perform \is{accent performance}ance} and metalinguistic comments).
...
Both men and women were interviewed and a rough socio-economic distinct \is{\isi{distinctness}}ion into working class or middle class was made.
...
On separate tiers of the Praat TextGrid, speaking style (word ``list'', ``reading'' (passage), ``free'' (speech), and (accent) ``imitation \is{accent perform \is{accent performance}ance}'') and topic (``childhood'', ``Manchester'', ``\isi{identity}'' etc.) coded, followed by a third one where the participant's speech was segmented into chunks and transcribed.
...
It is also rather salient \is{salience} and commented on by many non-linguists as \enquote*{g-dropping}.
...
The difference between [ŋ] and [ŋg] (or the devoiced \is{\isi{devoicing}} variant [ŋk]), on the other hand, exhibits the same kind of gradualness and, as explained above, can be measured in the same way as /k/ lenition.
...
Since linking up data from production and perception is a major interest of this study, the focus in the production part was also exclusively on the [ŋ]-[ŋg] distinct \is{\isi{distinctness}}ion.
Figure \ref{fig.automatic.consonants} shows two examples and their respective marking in the TextGrid.
	...
	\begin{figure}[h!]
		...
		\begin{subfigure}{0.75\textwidth}
			...
			\caption{plosive, \isi{PDF} = 18.47\% (03MMC33)}
		\end{subfigure}
		\begin{subfigure}{0.75\textwidth}
			\centering
			\includegraphics[width=\textwidth]{figures/like_fricative}
			\caption{fricative, \isi{PDF} = 81.84\% (36FWC20)}
		\end{subfigure}
...
The potential effect of vocal tract maturation, i.e. \isi{change}s to length and shape of the vocal tract over the course of an individuals lifetime, further complicates matters \parencite[cf.][440--441]{harrington2006}.
...
\begin{enumerate}
	...
	\item \label{enum.norm.phon}preservation (or improvement) of phoneme distinct \is{\isi{distinctness}}ions
...
\end{enumerate}
The author is well aware \is{\isi{awareness}} of the irony involved here.
...
Normalisation methods are generally categorise \is{categorisation}d with respect to two dimensions: vowel-intrinsic vs. vowel-extrinsic and speaker-intrinsic vs. speaker-extrinsic \parencite[cf.][]{kendallthomas2014}.
...
Since I am not aware \is{\isi{awareness}} of any evidence that suggests this is \emph{not} true for \textsc{fleece} and \textsc{trap} in Scouse, these two were used as corners in this study.
...
To counter potential skewing due to the fact that \textsc{trap} might not be exactly halfway between \textsc{fleece} and \textsc{goose} with respect to frontness, \([a]F_{2}\) is also derived instead of measured in the modified version of the algorithm employed in this study \parencites(cf.)()[420--421]{fabriciusetal2009}{kendallthomas2014}.
				...
				\begin{figure}[h!]
					...
					\begin{subfigure}{.49\textwidth}
						...
						\caption{Bark difference normalise \is{normalisation}d}
						\label{fig.plot.scatter.bark}
					\end{subfigure}
					\begin{subfigure}{.49\textwidth}
						\centering
						\resizebox{\linewidth}{!}{\input{figures/plot.scatter.watt.tex}}
						\caption{Watt-Fabricius normalise \is{normalisation}d}
...
In our case this would mean that phonemes should be more distinct \is{\isi{distinctness}} and that scatter around phoneme means should be reduced in the normalise \is{normalisation}d data.
When we look at Figure \ref{fig.plot.scatter.bark} this is not immediately obvious.
It should be borne in mind that with respect to \textsc{nurse} and \textsc{square} we are talking about a merger for most speakers so we would not necessarily expect these two phonemes to appear more distinct \is{\isi{distinctness}} in normalise \is{normalisation}d data.
...
\isi{Pillai} scores range between 0 and 1, with values close to 0 (and an accompanying high p-value) indicating a large degree of overlap between the distributions, and values near 1 (and a low p-value) representing distributions that are (almost) completely distinct \is{\isi{distinctness}}.
The pairing \textsc{nurse-square} is not represented in Table \ref{tab.pillai} as these two vowels participate in a merger for most speakers and it can therefore not be assumed that they \emph{should} be distinct \is{\isi{distinctness}} in the first place.

\begin{table}[h]
	\centering
	\caption{\isi{Pillai} scores for total and female/male vowel distributions}
	%\scriptsize
	\begin{tabular}{lllllll}
		\hline
...
So while the Bark difference \isi{normalisation} did not make different phonemes appear more distinct \is{\isi{distinctness}}, it did at least not result in significantly \emph{less} distinct \is{\isi{distinctness}} categories either.
\citeauthor{wattfabricius2002}, on the other hand, increases the distinct \is{\isi{distinctness}}ness of happ\textsc{y} to both \textsc{nurse} and \textsc{square}.
...
For all three vowels, female and male realisations are quite distinct \is{\isi{distinctness}} in the raw data, a fact which is reflected in \isi{Pillai} scores which are comparable or actually higher than those for different phonemes, and p-values that indicate highly significant differences. 
...
However, the figures show that although the gender distributions are less distinct \is{\isi{distinctness}} than in the Bark-difference normalise \is{normalisation}d data, they are still (and by a much larger factor than in the Bark values) more distinct \is{\isi{distinctness}} than the normalise \is{normalisation}d female/male distributions of happ\textsc{y}.
...
\begin{table}[h]
	\centering
	\caption{Euclidean distance ratios for raw and normalise \is{normalisation}d data}
	\begin{tabular}{llll}
		\hline
...
Style is the independent variable I am most interested in as the presence or absence of style-shifting is taken as an \isi{indicator} of \isi{salience} (cf. Chapter \ref{ch.sal}).
...
Secondly, the \isi{phonological context} had been restricted to Intervocalic \is{\isi{phonological context}} and word-final \is{phonological context} occurrences from the start, so it was considered unnecessary to enter information in the same way as it had been done for the two vocalic variables (i.e. `spread out' over four different independent variables).
Instead, phonological environment \is{phonological context} was summarised in a single predictor (``Environment'' in the spreadsheet), which was to code whether the measurement had been taken in an Intervocalic \is{\isi{phonological context}} context (within a word) or at the end of a word.
...
All statistical test were perform \is{accent performance}ed using the R software \parencite{R}.
...
functions written by Austin Frank\footnote{code downloadable from \url{hlplab.wordpress.com/2011/02/24/diagnosing-\isi{collinearity}-in-lme4/}}.