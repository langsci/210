...
The claim that younger Scousers' speech is noticeably more local \parencite[cf.][]{watson2007a} could be confirmed, but only for the two \isi{salient} variables in the sample (\textsc{nurse}-\textsc{square} and \isi{lenition}), which appear to carry considerable amounts of \isi{covert} \isi{prestige}.
Local variants of non-\isi{salient} variables, on the other hand, were actually found to be receding.
Young Liverpudlians seem to be somewhat more willing to express a \isi{local identity} linguistically than older ones, but they rely almost exclusively on highly \isi{salient} and/or stigmatised features for doing so.

Linguistic norms and attitudes in the \isi{speech community} have remained relatively stable.
Speakers of all three generations investigated generally like `soft' or light \isi{Liverpool} accents, but largely reject very strong ones, to a not inconsiderable degree because the latter are perceived as exaggerated and artificial.
Despite these similarities the presence of \isi{hypercorrection} particularly in the middle-aged speakers suggests that this group is most sensitive to the \isi{negative image} of \isi{Liverpool} and \isi{Scouse}, probably because economic decline and \isi{stigmatisation} of the city were at its historic height in the 1970's and 80's when these speakers were growing up.

The phonological variables investigated are not equally \isi{salient} in all three age groups.
...
With respect to the \textsc{nurse}-\textsc{square} \isi{merger}, however, \isi{conscious} and \isi{sub-\isi{conscious} awareness} declines from the middle to the young generation, while \isi{lenition} of /k/ sees a steady and linear increase in \isi{salience} from the oldest to the youngest speakers.
Crucially, however, \isi{lenition} of /k/ is the most \isi{salient} feature in \emph{all} age groups, and is universally followed by the \textsc{nurse}-\textsc{square} \isi{merger}, velar nasal plus, and happ\textsc{y}-tensing.
...
The \textsc{nurse}-\textsc{square} \isi{merger} and /k/ \isi{lenition}, on the other hand, both generate robust \isi{priming} effects, and for the latter \isi{salience} can even explain differences between sub-groups of stimuli (divided by \isi{phonological environment}) or subjects (middle vs. working class background).
The main hypothesis that this study was built on could thus be confirmed: The more socially \isi{salient} a linguistic variable is, the more pronounced the resulting effect in an \isi{exemplar} \isi{priming} experiment will be; below a certain level of \isi{sub-\isi{conscious} awareness} no statistically significant \isi{priming} effects are generated.

Intriguingly, all significant effects in the perception experiment are in the unexpected direction: Subjects who have been led to believe that the speaker is from \isi{Liverpool} are \emph{less} likely to perceive variants typical of \isi{Liverpool} English.
While it is seemingly at odds with existing \isi{priming} research in sociolinguistics, this result is actually compatible with previous work in psychology and suggests that the \isi{phonetic distance} between the \isi{prime} and the actual speech signal is too great for perceivers to include the stimulus in the primed category.
Priming works nevertheless, but the outcome is a \isi{contrast effect} instead of the assimilation effects that were found in the studies conducted in Detroit and New Zealand.
...
All the same, a different test design that is specifically aimed at investigating frequency effects in \isi{priming} experiments might be able to yield further interesting insights.
...
The \isi{phonetic distance} between the primed variety and the one actually used in the stimuli must be comparatively small, and \isi{categorisation} of the stimuli must be a comparatively difficult task to start with.
