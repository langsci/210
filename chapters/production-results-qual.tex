...
This seems like a rather drastic interpretation of the distinct\is{distinctness}ness of Scouse, and I cannot say whether this idea is embraced by a majority of Liverpudlian\is{identity}s, but one of the younger subjects even went one step further and admitted to feeling ``a sense of detachment sometimes'', supposedly ``'cause the accent's so different from the rest of England'' (02MWC20).
...
Notwithstanding that most speakers in the sample consider Scouse to be so characteristic of the city, Liverpudlian\is{identity}s of all age groups are also aware \is{awareness} of the fact that speakers of Scouse can be found outside of the city limits.
...
Incidentally, she also thinks that the latter's accent is ``the worst'', which serves to illustrate another aspect often connected with the term plastic Scouser\is{identity}, namely the idea that ``they put it on too much'', i.e. that they perform\is{accent performance} an inauthentic\is{authenticity} and exaggerated Scouse accent (33FMC38).
...
All the same, neither the ideas about the distinct\is{distinctness}ness of Scouse, nor the views expressed about so-called plastic Scouser\is{identity}s should be taken to mean that Liverpudlian\is{identity}s across the board necessarily think that their city is absolutely unique and not part of any larger cultural region.
...
This conditional and the past tense that follows it suggest that he believes this distinct\is{distinctness}ion is less important or pronounced these days.
...
The linguistic \isi{north-south divide}, if it exists, is thus likely to be a \emph{social} distinct\is{distinctness}ion that just happens to coincide with a geographical split, due to the fact that social segregation has been present in Liverpool for a long time already.
...
Figure \ref{fig.aware.nurse} summarises aware \is{awareness}ness of the \textsc{nurse}-\textsc{square} merger in the three age groups under scrutiny in this thesis.
...
It is not unlikely that younger Scouser\is{identity}s are particularly aware \is{awareness} of this variable because it is not just a \isi{shibboleth} for Liverpudlian\is{identity}s, but also for outsiders.
...
Awareness of lenition thus increases in a near-linear fashion from the oldest to the youngest participants: it starts out as a feature which only a handful of Liverpudlian\is{identity}s are aware \is{awareness} of, gains dramatically in prominence in the middle group, and finally reaches a state of full conscious\is{awareness} aware \is{awareness}ness in the youngest generation of speakers interviewed.

Having said all that, it should be noted that it is quite possible that Scouser\is{identity}s are \emph{generally} aware \is{awareness} of the variable but not, for some reason, of its presence \emph{in their own speech}.
...
The distinct\is{distinctness}ion into stronger and lighter accents is not expressly made in this quote, but it is paraphrased as `over-emphasis', on the one hand, and `unaffected' and `natural', on the other.
...
Another speaker explains that, to him as an insider, ``it sounds friendly, but [he doesn't] know what it sounds, like, from outside looking in'' and he can well imagine that, in the latter case, ``it could be a bit intimidating sometimes'' (25MMC19) --- which is particularly interesting because it is an almost word-by-word repetition of something another Liverpudlian\is{identity}s said who is more than 45 years older and from a different social class (cf. Section \ref{aware_res.eval.old.like}).
...
A different male working-class speaker talks about the same issue --- `plastic' Scouse accents --- in the context of the media (where conscious\is{awareness} accent perform\is{accent performance}ances are much more likely to occur than in `real life').
...
Interestingly (but probably not too surprisingly), a speaker from the middle-aged group used John Bishop as a prime\is{priming} example of a plastic Scouser\is{identity}, so it is presumably controversial among Liverpudlian\is{identity}s where exactly the `plastic' line is to be drawn.
...
Nevertheless, they remark that Scouse is ``not [their] favourite accent'', an attitude which is likely to be influenced by the aware \is{awareness}ness that, from an external point of view, the Liverpool accent is among ``the most hated in England'' (07FMC23).
