...
The platform \citetitle{sosci} does come with some technical limitation\is{accent performance}s, however, since it is not primarily designed for tests requiring RT measurements.
...
The claim that at least some expectations and attitudes will prime\is{priming} people to perceive particular sounds \emph{implies} that at least to a certain extent the choice is already made before subjects can really process the physical signal and this might well show in negative RTs.
...
After all, \isi{exemplar} \isi{priming} can only work if there are \isi{exemplar}s that can be activated by the prime\is{priming}.
...
The remaining participants seem to come more or less from all over the country, although Wales\is{Welsh} and Northern Ireland\is{Irish} are clearly under-represented.
