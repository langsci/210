...
\subsection{Stimuli sentences and \isi{frequency} of keywords}
...
For /ŋ(g)/ and /k/, a further criterion was to have an equal number of words with the variable in Intervocalic \is{\isi{phonological context}} and in word-final \is{phonological context} position.
...
Frequency \is{frequency} \isi{categorisation} was initially based on occurrences in the BNC (spoken language section only).
Table \ref{tab.keywords.\isi{frequency}} provides an overview of the keywords used.
...
The actual equation used in practice is slightly more complex in order to also assign Zipf \is{Zipf score} scores to words that have an absolute \isi{frequency} of 0 in the corpus, but this does not drastically \isi{change} the interpretation of the Zipf \is{Zipf score} scores as outlined just above \parencite[cf.][]{heuvenetal2014}.
...
\begin{table}[h]
	...
	\label{tab.keywords.\isi{frequency}}
	\centering
...
The author is aware \is{\isi{awareness}} of the fact that this list probably leaves much to be desired from the point of view of linguistic \isi{frequency} research.
...
An even number of stimuli was chosen to `force' listeners to one side of the spectrum (more Mancunian or more Liverpudlian \is{identity}).
...
An LPC (prediction order 12, window length 25 milliseconds, time step 5 milliseconds, pre-emphasis \isi{frequency} 50 Hz) and a formant object (maximum number of formants 5, maximum formant \isi{frequency} 5500 Hz, window length 25 milliseconds, pre-emphasis \isi{frequency} 50 Hz) were created, both using the Burg algorithm.
...
\begin{table}[h]
	\caption{Filter settings used in vowel \isi{resynthesis \is{resynthesis}} (in Hz)}
...
The sound files that were played had been created using another Praat script written by the author which pasted the stimulus sentence together with the \isi{resynthesis \is{resynthesis}}ed keywords and added bits of silence in between (Figure \ref{fig.per.sentence} visualises the structure).
...
The platform \citetitle{sosci} does come with some technical limitation \is{accent perform \is{accent performance}ance}s, however, since it is not primarily designed for tests requiring RT measurements.
...
The second, more important reason, is that it does not necessarily make sense to exclude an answer just because the subject did not listen to all \isi{resynthesis \is{resynthesis}}ed sounds first.
...
The claim that at least some expectations and attitudes will prime \is{\isi{priming}} people to perceive particular sounds \emph{implies} that at least to a certain extent the choice is already made before subjects can really process the physical signal and this might well show in negative RTs.
...
Some ex\isi{change} students were recruited with the help of public notice boards on the campus of Freiburg University.
...
After all, \isi{exemplar} \isi{priming} can only work if there are \isi{exemplar}s that can be activated by the prime \is{\isi{priming}}.
...
Among middle class subjects, female and male participants are not really evenly distributed across \isi{priming} conditions: women dominate in the group that was prime \is{\isi{priming}}d for Liverpool, while men do in the other.
...
\begin{table}[h]
	...
	\begin{tabular}{lcccc}
		\hline
		prime \is{\isi{priming}} & \multicolumn{2}{c}{`Liverpool'} & \multicolumn{2}{c}{`Manchester'}\\
		& F & M & F & M\\
		\hline
...
While answers are expressed in numbers (token 1, 2, 3, or 4), these are really labels for 4 distinct \is{\isi{distinctness}} categories rather than measurements on a continuous scale.
...
All two-way interactions of the prime \is{\isi{priming}} with these linguistic and extra-linguistic factors were also considered.
