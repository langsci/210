...
Based on a detailed analysis of production data, the first part of this study has independently established the sociolinguistic \isi{salience} of happ\textsc{y}-tensing, velar nasal plus, the \textsc{nurse}-\textsc{square} \isi{merger}, and \isi{lenition} of /k/.
Knowing they can be ordered in this way (from least to most \isi{salient}) it is now possible to test the hypothesis that \isi{salience} is a crucial factor in \isi{exemplar} \isi{priming} experiments.
...
For /ŋ(g)/ and /k/, a further criterion was to have an equal number of words with the variable in \isi{intervocalic} and in \isi{word-final} position.
This was to match the most prominent phonological contexts that these variables were investigated in in production and to be able to discover any potential differences in \isi{priming} related to this criterion.
...
Key words from the original list were selected in such a way that there was a continuum from very high to very low frequency words.
Frequency \isi{categorisation} was initially based on occurrences in the BNC (spoken language section only).
...
They propose an alternative measure called the \isi{Zipf} scale, the values of which are calculated as follows:
\begin{equation}
\text{Zipf score} = \log(n) + 3
\end{equation}
...
The actual equation used in practice is slightly more complex in order to also assign \isi{Zipf} scores to words that have an absolute frequency of 0 in the corpus, but this does not drastically change the interpretation of the \isi{Zipf} scores as outlined just above \parencite[cf.][]{heuvenetal2014}.
The \isi{Zipf} scores used in this study are based on the frequencies in SUBTLEX-UK as a whole (instead of one of the two sub-corpora `CBeebies' (pre-school children) and `CBBC' (primary school audience), for which separate \isi{Zipf} values are also available).
...
Ideally, there would have been two words each in more clearly (and more consistently) defined high, middle and low frequency ranges for every variable investigated: \textcite{heuvenetal2014} draw the line between low frequency and high frequency words somewhere between \isi{Zipf} values 3 and 4, which would classify only 1 in 6 of the carrier words as low frequency and the remainder as high frequency.
...
It should be borne in mind, however, that frequency is only of secondary interest in this study, and the opportunity to possibly find connections between use and perception of particular words was deemed more relevant to the task at hand.
The whole design of the experiment would have looked different if frequency had been of central importance.
For instance, more keywords would be needed, because with only 6 words (per variable) chances are high that any effects of frequency that might be found are, at least in part, \emph{lexical} effects of individual words rather than (generalisable) frequency effects.
...
These sentences were recorded by a phonetician in her late twenties who is originally from East Central \isi{Manchester}.
She used her native \isi{Mancunian} accent, meaning that \textsc{nurse} was realised as [ɜ:], happ\textsc{y} as [ɪ] (phrase-internally) or [ə] (phrase-finally), velar nasal plus as [ŋg]\footnote{Using velar nasal plus as a variable in a \isi{priming} test where the conditions are `\isi{Liverpool}' and `\isi{Manchester}' is problematic, because velar nasal plus is actually present in both accents. See Section \ref{sec.perc_res.disc.issues} for an explanation why I still think it was justified to use this variable in this \isi{priming} experiment.} and /k/ as [k].
...
Firstly, creating more than 4 different realisations that can be reliably distinguished by non-experts would have been rather challenging in the case of velar nasal plus and /k/ \isi{lenition}.
Since it was deemed essential to ensure comparability between the \isi{consonantal} and the \isi{vocalic} variables, the number of answer tokens was therefore limited to 4 for the whole experiment.
...
An even number of stimuli was chosen to `force' listeners to one side of the spectrum (more \isi{Mancunian} or more \isi{Liverpudlian}).
...
The downsampled \isi{vowel} was then inverse-filtered with the LPC, which results in a `reconstruction' of the pure source sound as generated by the vocal folds of the speaker.
By applying the formant filter (which simulates vocal tract shape) to this reconstructed source a \isi{vowel} can be re-synthesised that is almost identical to the natural one in the recording (token 2 was created this way).
If the filter (i.e. the formant structure) is manipulated, a synthesised \isi{vowel} is generated that is acoustically and \isi{perceptually} different from the original, but most of the time still fairly natural sounding.
The re-synthesised \isi{vowel} was then upsampled again and spliced back into the carrier word.
...
There was thus a continuum from stimulus 1 (hyper-\isi{Mancunian}), to 2 (actual) and 3 (\isi{Scouse}), to stimulus 4 (hyper-\isi{Scouse}).
...
With respect to happ\textsc{y}, stimulus 1 was lowered and centralised, while stimuli 3 and 4 were higher and fronter than the \isi{vowel} that actually occurred in the recordings.
Participants always heard the re-synthesised words in the same order (1 to 4).
Unfortunately, it was not possible to \isi{synthesise} stimuli of satisfactory quality from the sentences that had happ\textsc{y} in phrase-final position (where it was typically realised as [ə], and often articulated with breathy voice).
...
The link between those two in the context of this study is that the proportion of friction/aspiration is higher in the \isi{Scouse} than in the standard British English variants, and this criterion was used to build a continuum similar to the one created for the two vowels.
Once more, the goal was to create roughly equal \isi{perceptual distance} (again checked \isi{auditorily} by the author) between any two tokens, just as for the re-synthesised vowels.
...
There was thus no voiced velar \isi{plosive} present in the sentences subjects heard, and the continuum of stimuli was structured in the same way as those for the vowels and for /k/ in the sense that stimulus number 2 was the objectively most accurate choice that corresponded best to the sound actually used in the sentence.

Stimulus 2 for /k/ was the actual released \isi{plosive} [k] as it occurred in the sentence recordings.
The hyper-\isi{Mancunian} stimulus was created from this by cutting the aspiration, but leaving in the burst.
The speaker had also recorded all the /k/ sentences using the \isi{Scouse} \isi{fricative} variants.
The \isi{frication} from these variants was pasted into the place of the original aspiration to form an \isi{affricate} stimulus 3 (to make the result more natural sounding half of the closure phase was also deleted).
Stimulus 4, finally, had the whole \isi{plosive} (silence, burst, and aspiration) replaced by the \isi{fricative} by way of removing remaining silence and burst from stimulus 3.

For both /ŋ(g)/ and /k/, asymmetrical cross-fading and intensity adaptation was applied in \isi{intervocalic} environments to create a smoother and less artificial transition from the nasal or the aspiration-less \isi{plosive} to the following \isi{vowel} in the hyper-\isi{Mancunian} tokens (using a 25 milliseconds fade-in interval for the \isi{vowel} and 50 milliseconds of overlap between phonemes when concatenating).
Table \ref{tab.consonant.stimuli} provides an overview of the \isi{consonantal} stimuli.
...
A small pilot study was run among linguists of the English Seminar at the University of Freiburg to make sure the stimuli (both the \isi{vocalic} and the \isi{consonantal} ones) were
\begin{inparaenum}[(a)]
...
The control group was (correctly) told that the speaker they were going to listen to was from \isi{Manchester}.
The other group was led to believe that the speaker was from \isi{Liverpool} (about 1 in 3 of the participants who lived outside of \isi{Liverpool} actually believed this).
Depending on which group subjects had been assigned to, `\isi{Manchester}' or `\isi{Liverpool}' was displayed at the top of every page as a reminder (cf. Figure \ref{fig.online.screenshot}).
...
For every sentence, subjects were asked to pay special attention to the (part of the) word that was underlined.
The sound files that were played had been created using another Praat script written by the author which pasted the stimulus sentence together with the \isi{resynthesised} keywords and added bits of silence in between (Figure \ref{fig.per.sentence} visualises the structure).
...
The second, more important reason, is that it does not necessarily make sense to exclude an answer just because the subject did not listen to all \isi{resynthesised} sounds first.
After all, this study is interested in finding out how (\isi{stereotypical}) expectations influence people's perception.
The claim that at least some expectations and attitudes will \isi{prime} people to perceive particular sounds \emph{implies} that at least to a certain extent the choice is already made before subjects can really process the physical signal and this might well show in negative RTs.
...
A call for participants was distributed through Hope, \isi{Liverpool}, and \isi{Manchester} University.
Announcements posted in \isi{Liverpool} and \isi{Manchester} related groups on Facebook resulted in some (but very few) responses.
...
Finally, personal contacts in \isi{Liverpool} and \isi{Manchester} and some of the people who had been interviewed about a year earlier were contacted and asked if they would like to participate.
...
Requirements 1 and 2 were set to make sure participants were at least likely to have some degree of experience with or knowledge of the accents of \isi{Liverpool} and \isi{Manchester}.
After all, \isi{exemplar} \isi{priming} can only work if there are exemplars that can be activated by the \isi{prime}.
Given what \textcite{montgomery2007} found with respect to the status of \isi{Scouse} in particular it seems not too far fetched to assume that people with British nationality (and native competence of English) have at least some exemplars indexed for `\isi{Liverpool}', even if these are only derived from (\isi{stereotypical}) media performances.
People from, say, the U.S. or Australia, on the other hand, will probably be a lot less familiar with the British accent landscape and might not have the slightest idea of what a \isi{Scouse} accent sounds like --- either because they have never listened to someone from \isi{Liverpool}, or because they have not done so \emph{knowingly}, meaning that the relevant exemplars will be indexed for a more general category such as `British'.
In both cases, \isi{priming} subjects for `\isi{Liverpool}' (or `\isi{Manchester}', for that matter) would not be possible.
...
Since this classification correlated strongly with the \isi{social class} participants had explicitly chosen anyway, self-reported \isi{social class} membership was used as a predictor for statistical modelling (see below).
...
\end{equation}
Where \(lon_L\) and \(lat_L\) are the longitude and latitude of central L1 (\isi{Liverpool} city centre), \(lon_s\) and \(lat_s\) the (central) coordinates of the subject's postcode, and \(d\) is the resulting distance value (the higher, the further the subject lives from \isi{Liverpool}).
...
The first is due to the fact that recruitment initially focussed on \isi{Liverpool} and \isi{Manchester}, while the London bias is to a degree even representative, given that between 15 and 20\% (depending on where one draws the boundaries) of the UK population live in the metropolitan area.
...
In addition to the more detailed placement via the postcode, participants were also assigned to one of the broad geographical categories ``internal'' (\isi{Liverpool} and \isi{Merseyside}, ``L'' postcode area; 9 subjects) and ``external'' (everything else, 58 subjects).
It is possible that some of subjects in the \isi{Liverpool} sample had already participated in one of the sociolinguistic interviews conducted by the author, but since these interviews had taken place (at least) a year earlier it is highly unlikely that this could have a distorting effect on results in the perception test.
...
Among middle class subjects, female and male participants are not really evenly distributed across \isi{priming} conditions: women dominate in the group that was primed for \isi{Liverpool}, while men do in the other.
However, Prime and Gender are still not collinear in the dataset (κ= 1.54), and mixed-effects ordinal models regressing reported \isi{percept} on Gender did not find a significant effect of the latter, neither in the `\isi{Liverpool}' (p = 0.801) nor in the `\isi{Manchester}' condition (p = 0.597).
...
On the other hand, the data \emph{are} ordered in a way (from most standard/\isi{Mancunian} to most \isi{Liverpool}), so while choosing token 4 instead of token 3 might not be the same as choosing 3 instead of 2 in terms of \isi{perceptual distance}, higher values do in all cases represent a more Liverpool-like \isi{percept} than lower values --- much in the same way that, say, a higher RT always mean that a subject took more time to respond than another one with a lower RT.
...
Carrier word was not included as a random effect because frequency and, for consonants, \isi{phonological environment} were of interest as fixed effects.
Since there were only 6 keywords per variable filtering out any lexical effects would almost certainly also have eliminated effects of frequency and/or \isi{phonological environment}.
Prime was entered as a fixed factor, along with gender, \isi{social class} (self-reported), age of the subject, and \isi{geographical distance} from \isi{Liverpool}.
Furthermore, the \isi{Zipf} score of the carrier word was included, as was the position of the carrier word in the stimulus sentence (sentence-medially or -finally), the position of the stimulus sentence within the experiment (when was the sentence played to the subject), and, for consonants, the \isi{phonological environment} (V\_V or \_\#).
All two-way interactions of the \isi{prime} with these linguistic and extra-linguistic factors were also considered.
...
\isi{Collinearity} was investigated applying the functions written by Austin Frank (cf. Section \ref{sec.prod_method.stats}) to corresponding mixed-effects logistic regression models.