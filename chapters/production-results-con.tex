...
Table \ref{tab.k.mean.environment} lists the exact means and the number of observations that were collected for each environment (note that this table is inverted relative to Figure \ref{fig.box.k.environment}, so contexts favouring \isi{lenition} are now found at the top).
...
Realising the phonological \isi{plosive} \isi{intervocalically} as a \isi{fricative} (produced with a narrow, but not blocked vocal tract) can therefore be seen as an (extreme) connected speech phenomenon motivated by articulatory economy.
...
We will start the analysis of social predictors by looking at the interaction of gender and style, which is visualised by Figure \ref{fig.box.k.stylegender}.
...
In light of this, I feel justified in claiming that \citeauthor{watson2007a}'s \citeyearpar{watson2007a} finding has been corroborated and that with respect to /k/, \isi{Scouse} is indeed ``getting \isi{Scouser}''.
...
Since the mixed linear effects regression model found significant three-way interactions of style and age group with gender and \isi{social class}, respectively, we will look at both of these as well before closing this section.
