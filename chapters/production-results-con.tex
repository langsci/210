...
Once more, by far the biggest share of the data stems from free speech (n = 5733), followed by the reading passage (n = 806), \isi{accent perform \is{accent performance}ance} (n = 640), and finally the word list (n = 390).
...
\begin{figure}[h]
	...
	\caption{/ŋ(g)/: \isi{PDF} by phonological environment \is{phonological context} (without [ɪn] realisations)}
	\label{fig.box.ng.environment}
\end{figure}

Figure \ref{fig.box.ng.environment} is a box plot that visualises \isi{PDF} of /ŋ(g)/ for the different \isi{phonological context}s separately.
...
\begin{inparaenum}[(1)]
	\item Comparatively high \isi{PDF} (word-final \is{phonological context}, Intervocalic \is{\isi{phonological context}}, followed by liquids),
	\item medium \isi{PDF} (Intervocalic \is{\isi{phonological context}} across word-boundary, followed by voiceless fricatives), and
...
\begin{table}[h]
	\centering
	\caption{/ŋ(g)/: \isi{PDF} means by phonological environment \is{phonological context} ([ɪn] excluded)}
...
The only exceptions are cases where velar nasal plus occurs Intervocalic \is{\isi{phonological context}}ally within a word or in phrase final position (``V\_\#''), i.e. when the variable is followed by silence.
...
Secondly, only the two environments `Intervocalic \is{\isi{phonological context}}' (within a word) and `word-final \is{phonological context}' (pre-pausal) occur in the word list.
...
Additionally, the Intervocalic \is{\isi{phonological context}} (within word) and word-final \is{phonological context} contexts are rather prominent in the sample \emph{generally}.
Together with Intervocalic \is{\isi{phonological context}} (across a word-boundary), an environment that is just as frequent as Intervocalic \is{\isi{phonological context}} (within word) and which also has a comparatively high mean \isi{PDF}, these two contexts  already account for 68.03\% of the /ŋ(g)/ observations ([ɪn] realisations excluded).
...
\begin{figure}[h]
	...
	\caption{/ŋ(g)/: \isi{PDF} by style and gender (without [ɪn] realisations)}
...
In the two remaining styles `reading' (t(284.859) = 4.212, p < 0.001) and `imitation \is{accent perform \is{accent performance}ance}' (t(140.233) = 2.228, p = 0.028), the gender difference is significant.
This is due to the fact that the means of men in both registers are lower than those of women (who have comparable means for the word list, the reading passage, and \isi{accent perform \is{accent performance}ance}).
...
\begin{table}[h]
	...
	\begin{tabular}{lrrrrrr}
		...
		list-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-0.350} & 117.131 & 0.727 & 2.908 & 134.609 & 0.004\\
		reading-free & 6.044 & 321.775 & < 0.001 & 1.502 & 224.352 & 0.135\\
		reading-imitation \is{accent perform \is{accent performance}ance} & 0.432 & 148.046 & 0.667 & \ensuremath{-0.482} & 144.108 & 0.630\\
		free-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-4.227} & 117.679 & < 0.001 & \ensuremath{-1.710} & 90.826 & 0.091\\
		\hline			
	\end{tabular}
\end{table}
...
When we look at the male subjects, we find that `reading', `free', and `imitation \is{accent perform \is{accent performance}ance}' are all statistically identical, `list' is the only one that is significantly different (cf. again Table \ref{tab.ng.genderstyle.pvalues} for the details of the relevant t-tests).
...
\begin{inparaenum}[(1)]
	...
	\item Men have comparatively low mean \isi{PDF}s in accent imitation \is{accent perform \is{accent performance}ance}, spontaneous speech, and the reading passage; only in the word list does the use of velar nasal plus increase significantly.
...
\begin{figure}[h]
	...
	\caption{/ŋ(g)/: \isi{PDF} by age ([ɪn] excluded)}
...
This means that younger Liverpudlian \is{identity}s are not getting ``more Scouse'' with respect to velar nasal plus, but rather they seem to be reversing the trend begun by the middle group of speakers, although not to the extent that their realisations are identical to those of their grandparents' generation.
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{/ŋ(g)/: \isi{PDF} by age and class ([ɪn] excluded)}
\end{figure}
...
When we look at Liverpudlian \is{identity}s aged between 30 and 55, social class also seems to matter, but now it is actually the working-class speakers who use less Scouse variants than their middle-class counterparts.
...
In the youngest speakers (panel on the right), finally, the class distinct \is{\isi{distinctness}}ion for this feature has disappeared.
...
All three regression lines are distinct \is{\isi{distinctness}} from each other.
...
It follows that the \isi{change} in the overall usage of velar nasal plus that was found for the pooled results is not only driven by, but actually restricted to middle-class Liverpudlian \is{identity}s; working-class Scouser \is{identity}s do not seem to have \isi{change}d at all.
...
\begin{figure}[h]
	...
	\caption{/ŋ(g)/: \isi{PDF} by style ([ɪn] excluded)}
...
On the other hand, there is an undeniable rise from spontaneous speech towards \isi{accent perform \is{accent performance}ance}.
...
Compared to free speech, realisations during the accent imitation \is{accent perform \is{accent performance}ance} are clearly significantly more Scouse, irrespective of age group; \isi{PDF} reaches about the same level as when people read out a text, a register which `imitation \is{accent perform \is{accent performance}ance}' is not significantly different from (cf. again the (lack of) overlap in the standard error whiskers in Figure \ref{fig.line.ng.tot}).
This graph is somewhat reminiscent of the corresponding figure that was generated for F1 measurements of happ\textsc{y} (Figure \ref{fig.line.f1w.happy.tot} on page \pageref{fig.line.f1w.happy.tot}), which also revealed more Scouse values for both the word list and accent imitation \is{accent perform \is{accent performance}ance} when compared to reading and spontaneous speech.
...
\begin{figure}[h]
	...
	\caption{/k/: \isi{PDF} by phonological environment \is{phonological context} (released only)}
	\label{fig.box.k.environment}
\end{figure}

I will start once more by describing the effect of phonological environment \is{phonological context}, which is illustrated in Figure \ref{fig.box.k.environment} (again flipped\ for better representation, cf. Figure \ref{fig.box.ng.environment}).
...
\begin{table}[h]
	\centering
	\caption{/k/: \isi{PDF} means by phonological environment \is{phonological context} (released only)}
...
Table \ref{tab.k.mean.environment} lists the exact means and the number of observations that were collected for each environment (note that this table is inverted relative to Figure \ref{fig.box.k.environment}, so contexts favouring lenition are now found at the top).
The high \isi{PDF} values in word-final \is{phonological context} and Intervocalic \is{\isi{phonological context}} (within word) environments should not really come as a surprise.
...
Lenition in Intervocalic \is{\isi{phonological context}} positions is also a common process from a typological point of view, and can be explained primarily on phonetic grounds. 
...
Realising the phonological plosive Intervocalic \is{\isi{phonological context}}ally as a fricative (produced with a narrow, but not blocked vocal tract) can therefore be seen as an (extreme) connected speech phenomenon motivated by articulatory economy.
As such it might attract less social attention than in other positions.
Since word boundaries are often non-existent in phonetic terms (i.e. two adjacent words are commonly articulated as one stretch of connected speech, without any silence in between), it is no wonder that high \isi{PDF} values are also found for V\_\#V and V\_\#glide environments, because phonetically speaking these contexts are largely identical to Intervocalic \is{\isi{phonological context}} occurrences of /k/ \emph{within} a word (glides, or semi-vowels, are, after all, really vowels in phonetic terms).
Table \ref{tab.k.mean.environment} also shows that, just as for velar nasal plus, the two Intervocalic \is{\isi{phonological context}} (within or across words) and the word-final \is{phonological context} environments are the most important in terms of absolute numbers (76.62\% of released observations).

V\_V and V\_\# are again the only two \isi{phonological context}s where observations in relevant numbers are available for all styles, but this fact was considered unproblematic for the same reasons that were outlined in Section \ref{sec.prod.res.con.ng.phon}.
Once we account for the fact that there are fewer observations for the oldest group of speakers across the board (cf. Section \ref{sec.prod_method.participants}), the proportional importance of these phonological environment \is{phonological context}s is also found to be comparable in all three age groups.
...
\begin{figure}[h]
	...
	\caption{/k/: \isi{PDF} by style and gender (released only)}
...
With the exception of accent imitation \is{accent perform \is{accent performance}ance} (in the rightmost panel), all boxes have a relatively large vertical extent.
...
Both genders use strongly lenited variants with \isi{PDF}s of 75 and higher more than 75\% of the time (cf. the position of the first quantiles in the rightmost panel of Figure \ref{fig.box.k.stylegender}).
...
\begin{table}[h]
	...
	\begin{tabular}{lrrrrrr}
		...
		list-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-8.881} & 205.652 & < 0.001 & \ensuremath{-6.201} & 170.062 & < 0.001\\
		reading-free & \ensuremath{-2.186} & 262.723 & 0.030 & 0.676 & 224.321 & 0.500\\
		reading-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-7.637} & 354.391 & < 0.001 & \ensuremath{-5.052} & 313.565 & < 0.001\\
		free-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-8.009} & 217.296 & < 0.001 & \ensuremath{-8.152} & 262.352 & < 0.001\\
		\hline			
	\end{tabular}
\end{table}

If we focus on the style dimension within the two gender subgroups separately, differences emerge as well.
The graph shows that accent imitation \is{accent perform \is{accent performance}ance} is clearly separate from the other three styles (the means are much higher than in any of the other registers).
...
Medians seem to be distinct \is{\isi{distinctness}}, and t-tests on the raw data (cf. Table \ref{tab.k.genderstyle.pvalues}) show that the differences in means are also statistically robust.
...
T-tests confirm that men have statistically identical /k/ realisations in free speech, the reading, and the word list task; only \isi{accent perform \is{accent performance}ance} is significantly different from these three (cf. again Table \ref{tab.k.genderstyle.pvalues}).
...
\begin{figure}[h]
	...
	\caption{/k/: \isi{PDF} by style and social class (released only)}
...
When we look at accent imitation \is{accent perform \is{accent performance}ance}, however, the class difference is no longer significant (t(301.161) = 0.351, p = 0.726), very much like the gender difference, which also disappeared in this speaking style.
All the same, there is a subtle difference: In linguistic terms, the class distinct \is{\isi{distinctness}}ion is slightly more pronounced than the gender one.
...
\begin{table}[h]
	...
	\begin{tabular}{lrrrrrr}
		...
		list-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-10.750} & 204.745 & < 0.001 & \ensuremath{-4.830} & 172.248 & < 0.001\\
		reading-free & 1.044 & 268.949 & 0.297 & \ensuremath{-0.444} & 217.617 & 0.658\\
		reading-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-9.651} & 357.991 & < 0.001 & \ensuremath{-3.468} & 317.741 & < 0.001\\
		free-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-14.745} & 292.559 & < 0.001 & \ensuremath{-4.290} & 185.533 & < 0.001\\
		\hline			
	\end{tabular}
\end{table}
...
We can see that, again, the \isi{accent perform \is{accent performance}ance} task produces /k/ realisations which are extremely Scouse and clearly separate from free speech, reading, and the word list.
...
For middle-class subjects, t-tests on the raw data (cf. Table \ref{tab.k.classstyle.pvalues}) confirm that all of them are significantly different from `imitation \is{accent perform \is{accent performance}ance}', but none of them is significantly different from the other two.
...
Working-class speakers thus have a three-way style distinct \is{\isi{distinctness}}ion for /k/: word list, reading/free speech, and \isi{accent perform \is{accent performance}ance}.
Middle-class interviewees, on the other hand, have statistically identical /k/ pronunciations for all three `traditional' styles, and only distinguish accent imitation \is{accent perform \is{accent performance}ance} from these.
...
In other words, they are more reluctant to use (pronounced) /k/ lenition even in more informal contexts, unless they are specifically\footnote{In a manner of speaking. Subjects were not asked to specifically use /k/ lenition.} asked to do so (see the value for the accent imitation \is{accent perform \is{accent performance}ance}).
...
\begin{figure}[h]
	...
	\caption{/k/: \isi{PDF} by age (released only)}
	\label{fig.box.k.tot}
\end{figure}
...
If we consider a plot of the raw data (Figure \ref{fig.box.k.tot}), however, the picture \isi{change}s.
...
In light of this, I feel justified in claiming that \citeauthor{watson2007a}'s \citeyearpar{watson2007a} finding has been corroborated and that with respect to /k/, Scouse is indeed ``getting Scouser \is{identity}''.
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{/k/: \isi{PDF} by age and gender (released only)}
\end{figure}
...
Women and men have clearly distinct \is{\isi{distinctness}} medians in this age group (no overlapping notches), but it is now the men who have lower \isi{PDF} values than the women.
The same goes for the mean, which is also lower for men than for women.
However, it should be noted that the two genders are somewhat less distinct \is{\isi{distinctness}} in this sub-sample due to increased variation in both groups (the interquartile ranges occupy virtually the same space).
...
\begin{inparaenum}[(a)]
	\item the gender distinct \is{\isi{distinctness}}ion has become (even) more statistically robust again (t(1339.877) = -4.982, p < 0.001), and
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{/k/: \isi{PDF} by age and social class (released only)}
\end{figure}
...
In the next generation (middle panel), things have already \isi{change}d.
...
For this age group, working-class speakers have a significantly higher \isi{PDF} (and thus more Scouse /k/ realisations) than middle-class Liverpudlian \is{identity}s (t(1253.101) = -6.982, p < 0.001).
...
For working-class Liverpudlian \is{identity}s, on the other hand, the order of age groups is completely reversed: Old speakers have a comparatively low estimated \isi{PDF}, subjects aged 30-55 are in the middle, and the youngest participants have a very high estimated \isi{PDF} of around 80.
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{/k/: \isi{PDF} by gender and social class (released only)}
\end{figure}
...
\begin{figure}[h]
	...
	\caption{/k/: \isi{PDF} by style and age (released only)}
...
Just as for the other three test variables, this relationship (as well as the two three-way interactions of style, age, and gender/social class) will be visualised by a line plot (Figure \ref{fig.line.k.tot}), which shows register on the x-, and average \isi{PDF} on the y-axis.
...
Only when we get to accent imitation \is{accent perform \is{accent performance}ance} do we see a real \isi{change}: \isi{PDF} values increase dramatically in both age groups.
This suggests that there has to be some (sub-conscious \is{awareness}) aware \is{\isi{awareness}}ness of the variable in these groups, but it must be very limited --- otherwise we would expect differences between the other styles as well.
...
During \isi{accent perform \is{accent performance}ance}, Scouser \is{identity}s aged between 19 and 29 reach the same level of lenition as the oldest interviewees.
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{/k/: \isi{PDF} by style, age group, and gender (released only)}
\end{figure}
...
Provided we ignore the word list, there is actually a near linear (and significant) rise from the text passage to spontaneous speech to \isi{accent perform \is{accent performance}ance}.
...
Mean \isi{PDF} is high in all registers, but there are still significant differences between the two blocks `list'/`free' and `reading'/`imitation \is{accent perform \is{accent performance}ance}': /k/ realisations are even more lenited in the latter case than in the former.
...
This is followed by a steep rise into \isi{accent perform \is{accent performance}ance} on the right-hand side of the graph, which is similar to the one found for women of this age group.
...
Women in particular have a virtually perfect straight line, running from the bottom-left to the top-right corner of Figure \ref{fig.line.k.fem}, just like we would expect for a socially salient \is{salience} variable.
...
\begin{inparaenum}[(1)]
	...
	\item the reading passage and free speech do not differ in a statistically robust way, so men only have a three-way style distinct \is{\isi{distinctness}}ion: word list, reading/spontaneous, \isi{accent perform \is{accent performance}ance}.
\end{inparaenum}
...
\begin{figure}[h]
	...
	\begin{subfigure}{.49\textwidth}
		...
		\label{fig.line.k.wc}
	\end{subfigure}
	\caption{/k/: \isi{PDF} by style, age group, and social class (released only)}
\end{figure}
...
The, by now familiar, steep rise to a very high \isi{PDF} for accent imitation \is{accent perform \is{accent performance}ance} is also present.
...
Old working-class speakers behave in very much the same way as the pooled age group in Figure \ref{fig.line.k.tot}: `list', `reading', and `free' are not significantly different, but there is a steep and statistically robust increase of \isi{PDF} during accent imitation \is{accent perform \is{accent performance}ance}.
...
Their working-class counterparts (dotted line in Figure \ref{fig.line.k.wc}) are interesting because they echo the phenomenon just described for old middle-class speakers: `list' and `reading' are identical, followed by a significant drop in \isi{PDF} towards spontaneous speech, and then by a significant rise for accent imitation \is{accent perform \is{accent performance}ance}.
...
The following increase in \isi{PDF} during \isi{accent perform \is{accent performance}ance} is then even (slightly) more extreme than for the other two age groups.
Somewhat surprisingly, working-class speakers (dashed line in Figure \ref{fig.line.k.wc}) do distinguish all four styles.
Not only is each of them significantly different from each of the other three, but there is also a steady increase in \isi{PDF} from left to right --- just as one would expect for a socially salient \is{salience} variable that people are aware \is{\isi{awareness}} of to a certain degree.
...
It should be noted, however, that this might just be due to the fact that young middle-class speakers have considerably lower \isi{PDF} values than working-class Liverpudlian \is{identity}s of the same age in the first three styles; young middle-class speakers might just try not to use /k/ lenition at all (as far as they are able to do so, cf. \ref{aware \is{\isi{awareness}}_res.phon.k}), irrespective of speaking style, unless they are told to do so.