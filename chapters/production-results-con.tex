...
When we look at the male subjects, we find that `reading', `free', and `imitation\is{accent performance}' are all statistically identical, `list' is the only one that is significantly different (cf. again Table \ref{tab.ng.genderstyle.pvalues} for the details of the relevant t-tests).
...
In the youngest speakers (panel on the right), finally, the class distinct\is{distinctness}ion for this feature has disappeared.
...
All three regression lines are distinct\is{distinctness} from each other.
...
Table \ref{tab.k.mean.environment} also shows that, just as for velar nasal plus, the two intervocalic\is{phonological context} (within or across words) and the word-final\is{phonological context} environments are the most important in terms of absolute numbers (76.62\% of released observations).
...
Medians seem to be distinct\is{distinctness}, and t-tests on the raw data (cf. Table \ref{tab.k.genderstyle.pvalues}) show that the differences in means are also statistically robust.
...
T-tests confirm that men have statistically identical /k/ realisations in free speech, the reading, and the word list task; only accent perform\is{accent performance}ance is significantly different from these three (cf. again Table \ref{tab.k.genderstyle.pvalues}).
...
All the same, there is a subtle difference: In linguistic terms, the class distinct\is{distinctness}ion is slightly more pronounced than the gender one.
...
For middle-class subjects, t-tests on the raw data (cf. Table \ref{tab.k.classstyle.pvalues}) confirm that all of them are significantly different from `imitation\is{accent performance}', but none of them is significantly different from the other two.
...
Working-class speakers thus have a three-way style distinct\is{distinctness}ion for /k/: word list, reading/free speech, and accent perform\is{accent performance}ance.
...
\begin{inparaenum}[(a)]
	\item the gender distinct\is{distinctness}ion has become (even) more statistically robust again (t(1339.877) = -4.982, p < 0.001), and
...
Only when we get to accent imitation\is{accent performance} do we see a real \isi{change}: \isi{PDF} values increase dramatically in both age groups.
...
During accent perform\is{accent performance}ance, Scouser\is{identity}s aged between 19 and 29 reach the same level of lenition as the oldest interviewees.
...
\begin{inparaenum}[(1)]
	...
	\item the reading passage and free speech do not differ in a statistically robust way, so men only have a three-way style distinct\is{distinctness}ion: word list, reading/spontaneous, accent perform\is{accent performance}ance.
\end{inparaenum}
...
Old working-class speakers behave in very much the same way as the pooled age group in Figure \ref{fig.line.k.tot}: `list', `reading', and `free' are not significantly different, but there is a steep and statistically robust increase of \isi{PDF} during accent imitation\is{accent performance}.
