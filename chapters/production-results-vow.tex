...
The majority of the data were collected during the spontaneous part of the interview (n = 5245), while the word list (n = 761), the reading passage (n = 1361), and \isi{accent perform \is{accent performance}ance} (n = 583) account for much smaller proportions.
...
Contrary to the sections that deal with F1 and F2 individually, the focus in \ref{sec.prod.res.vow.happy.pil} and \ref{sec.prod.res.vow.nurse.pil} will be much less on social predictors of vowel realisation other than age, and more on a summary synthesis \is{resynthesis} of the style shifting results and a --- very general --- contextualisation of happ\textsc{y} and \textsc{nurse} with \textsc{fleece} and \textsc{square}, respectively.
...
Or, put another way, if a vowel is going to be short, there is \emph{not enough} time for the tongue to move into a comparatively extreme position, so realisation of the vowel will be more central.
			...
			\subsubsection{Frequency \is{frequency}}
	...
	\begin{figure}[h!]
		...
		\caption{happ\textsc{y} (F1) by \isi{frequency} of carrier word}
...
If one considers the scales of the y-axes in Figures \ref{fig.scatter.f1w.happy.dur} and \ref{fig.scatter.f1w.happy.zipf} it becomes immediately obvious that both \isi{frequency} and duration have only a rather small influence on F1.
...
This plot shows very nicely that the class difference seems to be mostly driven by female speakers, because, for these subjects, the two lines are comparatively far apart (estimated F1 for middle-class women is just above 0.81, whereas the value for working-class females is about 0.77), and the standard deviations are clearly distinct \is{\isi{distinctness}} from one another.
For males, on the other hand, the estimates are relatively similar for both social classes, and, what is more, the standard errors are not distinct \is{\isi{distinctness}} (which is visualised by the grey, partially overlapping areas on the right hand side of the plot).
...
Remember that one of the hypotheses that this study intends to test is that ``Scouse is getting Scouser \is{identity}'', i.e. that younger speakers have more extreme local variants and/or use these local variants more often than older speakers.
...
Liverpudlian \is{identity}s between the ages of 19 and 29 are therefore actually the \emph{least} Scouse in the sample, because they have lower realisations of happ\textsc{y} than their parents' generation, who in turn use lower variants than the oldest speakers.
...
Only for female subjects are the lines clearly distinct \is{\isi{distinctness}} from one another.
...
It is interesting that men do not seem to have \isi{change}d much across these three generations; their means are comparatively similar.
...
Figure \ref{fig.line.f1w.happy.tot} represents the style dimension along the x-axis, starting with the word list on the left and going through the reading passage and free, spontaneous speech to the accent imitation \is{accent perform \is{accent performance}ance} task on the right of the graph.
...
Realisations during the \isi{accent perform \is{accent performance}ance} are then higher again, though not as high as when reading a word list (the rise is non-significant for the oldest speakers in the sample).
Older speakers thus only distinguish the word list style from the other three (which do not differ significantly), middle-aged subjects have similar realisations for the word list and the \isi{accent perform \is{accent performance}ance}, and the youngest interviewees distinguish reading and spontaneous speech together from both imitation \is{accent perform \is{accent performance}ance} and the word list.
...
\begin{table}[h!]
	\centering
	\caption{happ\textsc{y}: durations (ms) and \isi{frequency} (Zipf \is{Zipf score} scores) by style and age}
	\label{tab.dur.style.happy}
	\begin{tabular}{lrrrrrrrr}
		\hline
		& \multicolumn{2}{c}{list} & \multicolumn{2}{c}{reading} & \multicolumn{2}{c}{free} & \multicolumn{2}{c}{imitation \is{accent perform \is{accent performance}ance}}\\
...
It is clear, however, that this is only part of the story, and that some sub-conscious \is{awareness} shifting must be involved as well, because \isi{vowel duration}s are comparable (and certainly not considerably longer) during accent imitation \is{accent perform \is{accent performance}ance}, text reading, and free speech.
...

{
	...
	\begin{longtable}[c]{p{0.4\textwidth}rrrrrl}
		...
		STYLEimitation \is{accent perform \is{accent performance}ance}:AGE56-85:GENDERfemale & -0.01 & 0.04 & 1945.32 & -0.34 & 0.74 & \\ 
		...
		STYLEimitation \is{accent perform \is{accent performance}ance}:AGE30-55:GENDERfemale & 0.05 & 0.03 & 1944.32 & 1.61 & 0.11 & \\ 
...
With respect to the non-social predictors there are some \isi{change}s as well.
...
\begin{table}[h!]
	\centering
	\caption{happ\textsc{y}: durations by phonological environment \is{phonological context}}
...
For female speakers (boxes on the left-hand side of each panel), the three styles `reading', `free', and `imitation \is{accent perform \is{accent performance}ance}' have relatively similar realisations.
This is especially true for reading and \isi{accent perform \is{accent performance}ance}, where means are very close and notches overlap.
...
The p-value of 0.22 for reading-imitation \is{accent perform \is{accent performance}ance} (females) confirms that these two styles are not significantly different, whereas spontaneous speech differs from both reading (p = 0.008) and imitation \is{accent perform \is{accent performance}ance} (p < 0.001) in a statistically robust way.
Realisations of happ\textsc{y} elicited via the word list show slightly more variation around the mean, but are nevertheless clearly distinct \is{\isi{distinctness}} from all three remaining styles as both the box plot and the t-tests (cf. Table \ref{tab.happy.genderstyle.pvalues}) suggest.
...
\begin{table}[h!]
	...
	\begin{tabular}{lrrrrrr}
		...
		list-imitation \is{accent perform \is{accent performance}ance} & 3.994 & 141.852 & < 0.001 & 3.984 & 197.933 & < 0.001\\
		reading-free & \ensuremath{-2.661} & 244.712 & 0.008 & 2.362 & 234.779 & 0.019\\
		reading-imitation \is{accent perform \is{accent performance}ance} & 1.229 & 260.601 & 0.220 & \ensuremath{-4.279} & 279.877 & < 0.001\\
		free-imitation \is{accent perform \is{accent performance}ance} & 3.482 & 146.933 & < 0.001 & \ensuremath{-7.047} & 158.739 & < 0.001\\
		\hline			
	\end{tabular}
\end{table}

When we look at male speakers (right-hand boxes in Figure \ref{fig.box.f2w.happy.stylegender}), \isi{accent perform \is{accent performance}ance} and reading realisations are now clearly distinct \is{\isi{distinctness}} (p < 0.001), and reading and spontaneous speech, in turn, look as if their means were comparable.
...
The rightmost column of Table \ref{tab.happy.genderstyle.pvalues} indicates that (in the raw data!) men actually keep all four registers distinct \is{\isi{distinctness}}.
...
Women have a considerably higher mean F2 than men, and while they also exhibit quite a bit of variation (as illustrated by the vertical extent of the box), mean and median are almost identical for both the female and the male subjects, and the confidence intervals are very small and clearly distinct \is{\isi{distinctness}}.
...
Gender is thus only a significant factor in spontaneous speech and \isi{accent perform \is{accent performance}ance}, but not for reading of a text or a word list.
When it does play a role, its effect is not uniform: women are `more Scouse' in free speech, but `less Scouse' in the accent imitation \is{accent perform \is{accent performance}ance} task.
...
I am aware \is{\isi{awareness}} of the fact that the linear mixed-effects regression did not find a significant interaction of these two predictors.
...
For all three age groups, there is a sharp drop from the word list to reading and the other two styles (with the exception of `imitation \is{accent perform \is{accent performance}ance}' in the old group, which looks as if it might not be significantly different from the word list values in the same age group).
...
When we turn to Figure \ref{fig.happy.space.young}, which represents the data collected from the youngest speakers, the picture \isi{change}s only slightly.
...
The second point of interest in this graph is that the difference between the means of \textsc{fleece} and happ\textsc{y} is smallest for the middle-aged realisations, larger for the observations pertaining to the older subjects, and most pronounced in the sub-sample of Liverpudlian \is{identity}s aged between 19 and 29.
...
Once again, however, this distinct \is{\isi{distinctness}}ion is rather fine-grained as class differences are highly significant for both genders.
...
The regression plot also shows that all age groups seem to be significantly different from one another as far as the male speakers are concerned: the lines are clearly distinct \is{\isi{distinctness}} and standard deviations (grey areas) do not overlap.
...
It looks thus as if male Scouser \is{identity}s have constantly raised \textsc{nurse} over the time period investigated here (if only to a very small extent in absolute terms), while female speakers first slightly raised \textsc{nurse} from the oldest to the middle-aged speakers, only to return to the starting point again in the youngest group.
...
As reported above, middle-class speakers have higher F1 values (which translate to more open, i.e. more Scouse \textsc{nurse} variants) than working-class Liverpudlian \is{identity}s.
...
\textsc{nurse} productions of the oldest speakers, on the other hand, seem to be distinct \is{\isi{distinctness}} from the other two groups, both in the middle and the working-class sub-sample.
...
The young and the middle group have virtually identical values in three out of four speaking styles, only the oldest speakers are slightly more distinct \is{\isi{distinctness}}.
However, even that is mostly true for the word list.
While reading a text and during \isi{accent perform \is{accent performance}ance} all three groups have comparable F1 measurements.
...
If we were looking at Labovian style shifting, we would expect an \emph{upward} slope, i.e. \textsc{nurse} realisations becoming \emph{more} Scouse from the word list to imitation \is{accent perform \is{accent performance}ance}, instead of the opposite.
...
This cannot be called more than a subtle trend, however, as styles adjacent on the formality continuum are only rarely significantly different (`reading' and `free' in the young, `free' and `imitation \is{accent perform \is{accent performance}ance}' in the other two groups).
...
\begin{inparaenum}[(a)]
	...
	\item F1 of \textsc{nurse} is (just about) significantly lower in \isi{accent perform \is{accent performance}ance} (`imitation \is{accent perform \is{accent performance}ance}') than in spontaneous speech (`free').
\end{inparaenum}
...
Second, if we lump together the word list and the reading passage (which do not seem to be different in a statistically robust way), this decrease is also significant from `reading'/`word list' to `free', and from `free' to `imitation \is{accent perform \is{accent performance}ance}'.
...
As a sort of reversal of the pattern found for middle-aged working-class speakers, there is then an increase towards the values found for `imitation \is{accent perform \is{accent performance}ance}', a mean which is again only significantly different from spontaneous speech, but not the reading passage or the word list.
...
Judging from the plot, the difference between genders is already highly significant in the middle-class sample (left panel) because not only are the means of women and men clearly distinct \is{\isi{distinctness}}, but they are also virtually identical to the medians of the same category (which argues for normally distributed data), and the confidence intervals do not occupy the same space at all.
...
This is not surprising because it means that middle-class speakers use less Scouse variants than working-class Liverpudlian \is{identity}s, which is true for both female (t(815.432) = -18.076, p < 0.001) and male subjects (t(933.018) = -11.305, p < 0.001).
Social classes are less distinct \is{\isi{distinctness}} when we focus on male subjects only (the vertical distance between the regression lines is smaller), but it should be noted that this is a highly relative statement as the difference is statistically extremely robust for this sub-group of speakers, too.
...
In the middle-aged group, however, realisations are just as clearly distinct \is{\isi{distinctness}} as they are identical for the other two age groups.
...
If one examines the boxplots in Figure \ref{fig.box.f2w.nurse.styleclass}, which are sorted from most formal register (word list) on the left to least formal one (\isi{accent perform \is{accent performance}ance}) on the right, it is clear that the pattern is pretty much the same in all of them.
Boxes that visualise the data collected from middle-class interviewees (always the left one in each panel) are consistently lower than the corresponding ones for working-class speakers, which illustrates once again that working-class speakers have more Scouse \textsc{nurse} variants than middle-class Liverpudlian \is{identity}s (cf. Figure \ref{fig.scatter.f2w.nurse.genderclass}).
T-tests confirm for the means what the notches in the box plots do for the medians: the class difference is statistically significant for the word list (t(188.089) = -8.096, p < 0.001), the reading passage (t(255.085) = -5.524, p < 0.001), spontaneous speech (t(1087.269) = -17.359, p < 0.001), and accent imitation \is{accent perform \is{accent performance}ance} (t(153.537) = -4.744, p < 0.001).
...
\begin{table}[h!]
	...
	\begin{tabular}{lrrrrrr}
		...
		list-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-4.894} & 192.492 & < 0.001 & \ensuremath{-1.107} & 153.071 & 0.270\\
		reading-free & 3.870 & 209.460 & < 0.001 & \ensuremath{-1.254} & 216.848 & 0.211\\
		reading-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-4.684} & 233.144 & < 0.001 & \ensuremath{-4.039} & 157.841 & < 0.001\\
		free-imitation \is{accent perform \is{accent performance}ance} & \ensuremath{-9.346} & 172.419 & < 0.001 & \ensuremath{-3.684} & 101.284 & < 0.001\\
		\hline			
	\end{tabular}
\end{table}
...
Both classes have the most Scouse realisations in the \isi{accent perform \is{accent performance}ance}, but apart from that the order of styles is different (cf. Table \ref{tab.nurse.classstyle.pvalues} for tests of significance).
For middle-class speakers `imitation \is{accent perform \is{accent performance}ance}' is significantly more Scouse than `reading', which is in second place and followed by the word list (which does not differ significantly from `reading').
...
When we look at working-class subjects, the order from most Scouse to least Scouse is: `imitation \is{accent perform \is{accent performance}ance}', `list', `free', and `reading'.
The first and the last two of these four are not significantly different from each other, so that statistically speaking we have a more Scouse block `imitation \is{accent perform \is{accent performance}ance}/list', and a less Scouse pair `free/reading'.
...
Middle-aged speakers have thus a fronter \textsc{nurse} than the oldest subjects in this sample, and speakers in the youngest group have, in turn, a significantly fronter vowel than Liverpudlian \is{identity}s of their parents' generation.
...
This sharp increase in F2 values from free speech to accent imitation \is{accent perform \is{accent performance}ance} is also present (and just as significant) in the middle-aged group.
...
F2 values in the middle age group thus strongly hint at hypercorrect \is{\isi{hypercorrection}}ion (cf. \ref{sec.prod.res.vow.nurse.pil}).
Speakers in the young group, finally, exhibit a pattern which is closest to what we would expect if we assume there to be style shifting: F2 increases from reading a text to free speech and from spontaneous speech to accent imitation \is{accent perform \is{accent performance}ance}; both these increases are (just about in the former case) statistically robust.
...
Accent imitation \is{accent perform \is{accent performance}ance} is significantly (and drastically) fronter than the other three registers.
...
Word list and reading passage are not significantly different, F2 drops significantly in free speech, and rises significantly again in \isi{accent perform \is{accent performance}ance}.
...
For the youngest speakers in the sample the class distinct \is{\isi{distinctness}}ion is most interesting.
Middle-class speakers exhibit a statistically completely level line for the first three styles, followed by the same rise in F2 for `imitation \is{accent perform \is{accent performance}ance}' as in the old and the middle-aged subjects.
Working-class speakers, on the other hand, do not significantly distinguish free speech from \isi{accent perform \is{accent performance}ance}.
...
\begin{table}[h!]
	\centering
	\caption{\textsc{nurse}-\textsc{square}: \isi{Pillai} scores by age and style (groups)}
	...
	\begin{tabular}{lrrrrrr}
		...
		imitation \is{accent perform \is{accent performance}ance} & 0.016 & 0.533 & 0.007 & 0.727 & 0.014 & 0.398\\
...
The last line in this table (which reports the tests perform \is{accent performance}ed on all \textsc{nurse} and \textsc{square} observations within the respective age group, pooled across style) tells us that \textsc{nurse} and \textsc{square} distributions are (highly) significantly different in all three age groups investigated --- all p-values are well below the 5\% threshold.
...
\begin{figure}[h!]
	...
	\begin{subfigure}{.49\textwidth}
		...
		\caption{\isi{accent perform \is{accent performance}ance}}
...
\begin{inparaenum}[(1)]
	\item The range of variation (as represented by the surface area of the polygons) increases considerably from the word list to free speech, via accent imitation \is{accent perform \is{accent performance}ance} and the reading passage.
...
The distributions are indeed more distinct \is{\isi{distinctness}} than in the other registers, if only slightly so.
...
All the same, the average realisations of the two vowels actually become more similar (instead of more distinct \is{\isi{distinctness}}, as would be expected for a more formal register), because \textsc{square} is both raised and centralised and thus approaches \textsc{nurse}.
...
Only when we move from free speech to accent imitation \is{accent perform \is{accent performance}ance} (Figures \ref{fig.nurse.space.old.free} and \ref{fig.nurse.space.old.imit}), does the mean of \textsc{nurse} \isi{change} more drastically: it is considerably more front, and almost identical to \textsc{square}, which is fronted and raised as well.
...
But in contrast to the older generation, the lowest degree of variation is found for accent imitation \is{accent perform \is{accent performance}ance}, while the word list and reading passage seem to be roughly comparable in this respect.
However, it should be noted that the realisational spaces --- particularly when comparing the word list, reading, and \isi{accent perform \is{accent performance}ance} --- differ less in area in the middle-aged group, generally, which is to say that the range of variation depends less on style than in the old group.
...
If this register is once again taken as the benchmark, or baseline, both \textsc{nurse} and \textsc{square} \isi{change} when people are asked to put on a particularly strong Scouse accent (Figure \ref{fig.nurse.space.mid.imit}): The former is fronted, the latter slightly raised.
...
This results in more \emph{merged} distributions (cf. Table \ref{tab.pillai.nurse.agestyle}), instead of the more \emph{distinct \is{\isi{distinctness}}} ones that would normally be expected in a more formal speaking style.
...
Figure \ref{fig.nurse.space.young} visualises the realisational spaces of \textsc{nurse} and \textsc{square} for Liverpudlian \is{identity}s aged between 19 and 29.
...
The plots for the word list, the reading passage, and accent imitation \is{accent perform \is{accent performance}ance}, in turn, do not seem to differ at all in terms of polygon area.
...
Younger Scouser \is{identity}s also set themselves apart when it comes to the mean values of these two variables.
...
During \isi{accent perform \is{accent performance}ance} (Figure \ref{fig.nurse.space.young.imit}), \textsc{square} essentially remains in the same location, while \textsc{nurse} moves forward, but only a tiny little fraction (which is not surprising, given that it is already very front in free speech).
Going from spontaneous speech to reading out a text (Figure \ref{fig.nurse.space.young.read}) neither \textsc{nurse} nor \textsc{square} \isi{change} very much, although the latter \emph{is} slightly more central.
...
In summary, one can say that the youngest speakers in my sample do not only show less variation \emph{within} a particular speech style, but they also show next to no \isi{change} \emph{between} styles.
...
\begin{figure}[h!]
	...
	\caption{\textsc{nurse}: \isi{Pillai} scores by age and social class (individuals)}
...
However, in the middle-aged group this distinct \is{\isi{distinctness}}ion has already become a lot more fuzzy: one half of the participants in this sub-sample has \isi{Pillai} scores near 0, the other half produces scores that are considerably higher (while still comparatively low in absolute terms).
Crucially, these scores no longer correlate with class, because both the merged and the distinct \is{\isi{distinctness}} sub-group include both middle \emph{and} working-class speakers.
