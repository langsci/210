...
This plot shows very nicely that the class difference seems to be mostly driven by female speakers, because, for these subjects, the two lines are comparatively far apart (estimated F1 for middle-class women is just above 0.81, whereas the value for working-class females is about 0.77), and the standard deviations are clearly distinct\is{distinctness} from one another.
For males, on the other hand, the estimates are relatively similar for both social classes, and, what is more, the standard errors are not distinct\is{distinctness} (which is visualised by the grey, partially overlapping areas on the right hand side of the plot).
...
Only for female subjects are the lines clearly distinct\is{distinctness} from one another.
...
Realisations of happ\textsc{y} elicited via the word list show slightly more variation around the mean, but are nevertheless clearly distinct\is{distinctness} from all three remaining styles as both the box plot and the t-tests (cf. Table \ref{tab.happy.genderstyle.pvalues}) suggest.
...
When we look at male speakers (right-hand boxes in Figure \ref{fig.box.f2w.happy.stylegender}), accent perform\is{accent performance}ance and reading realisations are now clearly distinct\is{distinctness} (p < 0.001), and reading and spontaneous speech, in turn, look as if their means were comparable.
...
The rightmost column of Table \ref{tab.happy.genderstyle.pvalues} indicates that (in the raw data!) men actually keep all four registers distinct\is{distinctness}.
...
I am aware \is{awareness} of the fact that the linear mixed-effects regression did not find a significant interaction of these two predictors.
...
Once again, however, this distinct\is{distinctness}ion is rather fine-grained as class differences are highly significant for both genders.
...
The regression plot also shows that all age groups seem to be significantly different from one another as far as the male speakers are concerned: the lines are clearly distinct\is{distinctness} and standard deviations (grey areas) do not overlap.
...
\textsc{nurse} productions of the oldest speakers, on the other hand, seem to be distinct\is{distinctness} from the other two groups, both in the middle and the working-class sub-sample.
...
If we were looking at Labovian style shifting, we would expect an \emph{upward} slope, i.e. \textsc{nurse} realisations becoming \emph{more} Scouse from the word list to imitation\is{accent performance}, instead of the opposite.
...
Judging from the plot, the difference between genders is already highly significant in the middle-class sample (left panel) because not only are the means of women and men clearly distinct\is{distinctness}, but they are also virtually identical to the medians of the same category (which argues for normally distributed data), and the confidence intervals do not occupy the same space at all.
...
Social classes are less distinct\is{distinctness} when we focus on male subjects only (the vertical distance between the regression lines is smaller), but it should be noted that this is a highly relative statement as the difference is statistically extremely robust for this sub-group of speakers, too.
...
In the middle-aged group, however, realisations are just as clearly distinct\is{distinctness} as they are identical for the other two age groups.
...
F2 values in the middle age group thus strongly hint at hypercorrect\is{hypercorrection}ion (cf. \ref{sec.prod.res.vow.nurse.pil}).
...
For the youngest speakers in the sample the class distinct\is{distinctness}ion is most interesting.
...
However, in the middle-aged group this distinct\is{distinctness}ion has already become a lot more fuzzy: one half of the participants in this sub-sample has \isi{Pillai} scores near 0, the other half produces scores that are considerably higher (while still comparatively low in absolute terms).
