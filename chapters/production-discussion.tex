...
Qualitative analysis of comments about \isi{identity} (cf. \ref{aware_res.north}) revealed that older subjects often consider Liverpool to be `unique' or `distinct\is{distinctness}' from the rest of England, both north and south.
...
This argument is rather speculative, but it is striking that women's mean and median F2 during accent perform\is{accent performance}ance are virtually identical to the values that men have in spontaneous speech (cf. Figure \ref{fig.box.f2w.happy.stylegender} on page \pageref{fig.box.f2w.happy.stylegender}).
...
In fact, \isi{vowel duration}s were somewhat shorter during imitation\is{accent performance} only for the youngest speakers in the sample, the rest had happ\textsc{y} pronunciations of similar length in text reading, spontaneous speech, and accent imitation\is{accent performance} (cf. Table \ref{tab.dur.style.happy}).
...
If one only looked at this result in isolation it would be tempting to conclude that there is little style shifting and therefore hardly any aware\is{awareness}ness of the variable.
...
In fact, young middle-class Scousers have the flattest line in the sample, i.e. they have a smaller amount of style shifting than any other group (plus there is no significant drop of F2 in free speech due to hypercorrect\is{hypercorrection}ion in `reading' and `list').
...
Data on style shifting (particularly when \textsc{square} realisations are also considered) and conscious\is{awareness} aware\is{awareness}ness clearly show that the merger is not equally salient\is{salience} in the three age groups of speakers.
For the oldest speakers it is a \isi{marker}, aware\is{awareness}ness of which is only just beginning.
In the middle generations, not only style shifting but also hypercorrect\is{hypercorrection}ion is widespread.
...
Some additional support for this interpretation can be found in \textcite[cf.][101]{newbrook1999}: In West \isi{Wirral} a not insignificant number of speakers endorsed realisations containing a plosive in both word-final\is{phonological context} and particularly in intervocalic\is{phonological context} position, probably because of ``sheer ignorance or confusion as to what the RP form might actually be''.
...
But of course the accent imitation\is{accent performance} task was still a highly artificial context and speakers presumably paid a lot of attention to their speech, albeit not in the traditional Labovian sense of the phrase.
All the same, getting the \isi{stereotype} `right' required them to focus on how they were articulating because this stereotypical\is{stereotype} accent was not their natural one (as is evidenced by the many comments about `falseness', cf. \ref{aware_res.eval}).
...
The fact that women are more Scouse than men in the middle group might therefore suggest that the variable has lost \isi{salience} in this generation, but additional evidence refutes this hypothesis (see \ref{prod.disc.k.aware}).
...
An additional relevant point here is that when the data are split up along the gender dimension, the youngest speakers are still the ones that show the most systematic style shifting pattern, and this is true for both women \emph{and} men, which shows that the increase in aware\is{awareness}ness along the age dimension is not limited to just one of the two genders, but is really primarily a question of age.
The fact that style shifting patterns are least different in the young group is further evidence for the idea that aware\is{awareness}ness of lenition is more universal in this generation than in the other two.
...
This looks very much like the hypercorrect\is{hypercorrection}ion that was observed for \textsc{nurse} (cf. \ref{prod.disc.ng.style}), but it is difficult to see why it would affect these two sub-groups in particular.
...
However, this would lead to the chicken and egg problem commonly encountered in \isi{salience} research (cf. \ref{sec.sal.sal.circle}): if aware\is{awareness}ness goes up because usage changes, then what triggers \isi{change} in usage in the first place?
Whatever the reason for the increase in conscious\is{awareness} aware\is{awareness}ness is, explicit comments clearly show that lenition of /k/ is even more of a \isi{stereotype} for younger Liverpudlians than it is for the older generations.
...
Inter-group differences in style shifting suggest that for the old and the middle-aged speakers (sub-conscious\is{awareness}) aware\is{awareness}ness is lower than in the young group because the former show only limited style aware\is{awareness}ness, while the latter present a textbook case of Labovian style shifting, with non-standard variants getting consistently and linearly more likely the more informal the communicative context.

Data on explicit comments and judgements further showed that conscious\is{awareness} aware\is{awareness}ness increases in a linear fashion from the oldest to the youngest Scousers.
...
\begin{inparaenum}[(1)]
	...
	As the feature that is clearly a \isi{stereotype} and even a \isi{shibboleth} for many, or even most, Liverpudlians it is without a doubt the most socially salient\is{salience} variable investigated in the context of this thesis.
\end{inparaenum}