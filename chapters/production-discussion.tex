...
\section{happ\textrm{\textsc{y}}: Indicator \is{indicator} (of northernness)}
...
I suspect that young Scouser \is{identity}s use laxer happ\textsc{y} variants for the same reason.
Qualitative analysis of comments about \isi{identity} (cf. \ref{aware \is{\isi{awareness}}_res.north}) revealed that older subjects often consider Liverpool to be `unique' or `distinct \is{\isi{distinctness}}' from the rest of England, both north and south.
...
Younger Scouser \is{identity}s often readily embrace a northern \isi{identity} as a means of setting themselves apart from the south and, at the same time, associating with other northern cities that they perceive as (more) similar to Liverpool.
...
Centralising happ\textsc{y} can be seen as a way of linguistically expressing solidarity with other northern cities and keeping one's distance from `the south', a region that many Liverpudlian \is{identity}s consider to be both geographically and culturally distant.
...
This could be a hint that women are at least sub-conscious \is{awareness}ly aware \is{\isi{awareness}} of the fact that men actually have more central variants than they themselves in spontaneous speech.
If we assume that the typical Scouser \is{identity} people think of when they are asked to perform \is{accent performance} the accent is male (which does not seem too far-fetched, given the negative \isi{stereotype}s associated with Liverpool), then one can interpret women's happ\textsc{y} realisation during accent imitation \is{accent perform \is{accent performance}ance} as more `realistic' than `stereotypical \is{stereotype}'.
This argument is rather speculative, but it is striking that women's mean and median F2 during \isi{accent perform \is{accent performance}ance} are virtually identical to the values that men have in spontaneous speech (cf. Figure \ref{fig.box.f2w.happy.stylegender} on page \pageref{fig.box.f2w.happy.stylegender}).
...
This result is diametrically opposed to \textcite{flynn2010}'s finding, because in his study (young) working-class women were the ones that drove the \isi{change} towards ultra-lax happ\textsc{y} variants, whereas in my data, these speakers seem to be the ones that have the most tense variants.
The impact of social class on F2, as mentioned above, depends on the age of the participant: in the old and the middle-aged group, working-class speakers have fronter vowels, but among the youngest speakers the effect is reversed and working-class Scouser \is{identity}s actually have more central variants.

Women having more standard-like realisations is in line with what many sociolinguists have found in many different contexts, but it is unclear why working-class women in particular would have more standard realisations than their middle-class counterparts --- unless they were hypercorrect \is{\isi{hypercorrection}}ing, which is not particularly likely for a largely non-salient \is{salience} (see below) variable.
...
\subsection{Style shifting and aware \is{\isi{awareness}}ness}
...
The lower F1 and higher F2 values in the word list readings could be explained phonetically (slower and clearer articulation, resulting in more peripheral vowels generally), but this is difficult for \isi{accent perform \is{accent performance}ance}, where the same trend (of more peripheral realisations) was observed.
People seem to believe, as explicit comments revealed, that speaking \emph{fast} is a typical feature of Scouse, so provided they incorporate this aspect into their \isi{stereotype} perform \is{accent performance}ance it would rather favour \emph{laxer} realisations of happ\textsc{y} instead of tenser ones.
In fact, \isi{vowel duration}s were somewhat shorter during imitation \is{accent perform \is{accent performance}ance} only for the youngest speakers in the sample, the rest had happ\textsc{y} pronunciations of similar length in text reading, spontaneous speech, and accent imitation \is{accent perform \is{accent performance}ance} (cf. Table \ref{tab.dur.style.happy}).
...
The approach of two conflicting norms that pull in the same direction might seem slightly unsatisfying, but some sort of very vague sub-conscious \is{awareness} aware \is{\isi{awareness}}ness of happ\textsc{y} as a feature of Liverpool English must be assumed if the increase in height and frontness in the imitation \is{accent perform \is{accent performance}ance} register is to be explained.
...
\end{inparaenum}
Part of the problem is certainly that ``the \isi{dialect}ological facts are complex and the interpretation of responses is often debatable'', and also that many subjects seemed to be confused ``as to what the RP form might actually be'', which is his explanation for the fact that the majority of his participants endorsed [i] despite the fact that this was still a non-standard variant in 1980 when he collected his data \parencite[101]{newbrook1999}.
On the basis of my data at least, \emph{conscious \is{awareness}} aware \is{\isi{awareness}}ness is out of the question: Not a single participant mentioned happ\textsc{y}-tensing as a typical feature of Liverpool English, or otherwise commented on it.
...
At the same time the distance between the means decreases, which means that for young Liverpudlian \is{identity}s \textsc{nurse} and \textsc{square} are considerably less distinct \is{\isi{distinctness}} than for middle-aged and old Scouser \is{identity}s.
...
This is thus a mild case of hypercorrect \is{\isi{hypercorrection}}ion, because by centralising \textsc{square} (which makes it \emph{less} standard) instead of \textsc{nurse} (which would become \emph{more} standard), people are actually moving the `wrong' vowel.
...
For the reading passage, middle-aged Liverpudlian \is{identity}s adjust the vowels in the same way as the old generation.
...
Speakers of the middle generation can therefore be said to present a textbook case of hypercorrect \is{\isi{hypercorrection}}ion, because their behaviour results in the opposite of what they are presumably trying to achieve: \textsc{nurse} and \textsc{square} pronunciations are even more non-standard in formal registers than they already are in spontaneous speech.
This suggests both heightened aware \is{\isi{awareness}}ness of the social meaning of this merger (hence the urge to modify usage according to communicative situation) and also a certain degree of \isi{linguistic insecurity} with respect to this variable.
...
\begin{inparaenum}[(1)]
	...
	\item speakers with comparatively high \isi{Pillai} scores, who keep \textsc{nurse} and \textsc{square} distinct \is{\isi{distinctness}}.
\end{inparaenum}
The crucial finding here is that, for the oldest speakers, higher \isi{Pillai} scores correlate with higher social status, because it is the middle-class speakers who maintain a distinct \is{\isi{distinctness}}ion and the working-class participants who are (more) merged --- just as one would expect in the early phases of the social life cycle of a linguistic variable.
In the middle-aged group there are both middle \emph{and} working-class speakers among the merged and the distinct \is{\isi{distinctness}} subjects, which shows that aware \is{\isi{awareness}}ness has spread to at least some working-class speakers (who then try to keep the two vowels more distinct \is{\isi{distinctness}}) and also that, in the middle class, speakers have started hypercorrect \is{\isi{hypercorrection}}ing, possibly because social aware \is{\isi{awareness}}ness (and \isi{stigmatisation}) of this variable has increased for them as well.
...
It would appear strange that women should use more Scouse variants of a salient \is{salience} variable, but if we remember that it is precisely the middle age group that was found to hypercorrect \is{\isi{hypercorrection}}, then this actually makes sense.
If a group of speakers is aware \is{\isi{awareness}} of a non-standard feature and so eager to avoid it that they develop a tendency to modify it in the `wrong' direction then it should come as no surprise that that tendency is actually more pronounced for women, given that female speakers are generally held to be more sensitive to linguistic differences that carry social meaning.
...
Interestingly, middle-class speakers of both genders seem to have lower, more Scouse, vowels than working-class Liverpudlian \is{identity}s.
...
A similarly clear distinct \is{\isi{distinctness}}ion is found with social class: The downward trend towards less Scouse variants the more informal the communicative context is more pronounced for middle-class subjects, particularly for the middle age group.
This is again in line with previous research: Female and middle-class speakers exhibiting more style shifting is just what is to be expected for a salient \is{salience} variable.
...
In principle, this holds for both social classes, the differences in style shifting (which is to say the \isi{change}s between styles, not the absolute values!) are only marginal.
...
If one only looked at this result in isolation it would be tempting to conclude that there is little style shifting and therefore hardly any aware \is{\isi{awareness}}ness of the variable.
However, we know from looking at \textsc{nurse} realisations in relation to \textsc{square} that the middle-age group is actually very prone to hypercorrect \is{\isi{hypercorrection}}ion: they do manipulate \emph{both} vowels in a consistent way, which is just not the expected one; \textsc{nurse} is progressively fronted the more formal the register.
...
The fact that both middle \emph{and} working-class speakers hypercorrect \is{\isi{hypercorrection}} underlines this by showing that aware \is{\isi{awareness}}ness of, and \isi{linguistic insecurity} relating to this merger seem to be universal in this age group.

The youngest speakers, finally, have steadily increasing (and significantly different) F2 values from reading out a text to free speech and accent imitation \is{accent perform \is{accent performance}ance}.
...
It seems thus as if younger Scouser \is{identity}s actually style-shift more consistently than older Liverpudlian \is{identity}s, which would be in stark contrast with the evidence discussed in Sections \ref{prod.disc.nurse.age} and \ref{prod.disc.nurse.social}, where I argued that \isi{salience} was \emph{de}creasing for the youngest Scouser \is{identity}s.
...
Middle- and working-class Scouser \is{identity}s aged 29 and younger behave differently, and this difference is not just one of degree.
Rather, working-class speakers contribute the (hypercorrect \is{\isi{hypercorrection}}) fronting in the word list style, while the middle-class subjects are responsible for the steep rise of F2 during \isi{accent perform \is{accent performance}ance}.
...
In fact, young middle-class Scouser \is{identity}s have the flattest line in the sample, i.e. they have a smaller amount of style shifting than any other group (plus there is no significant drop of F2 in free speech due to hypercorrect \is{\isi{hypercorrection}}ion in `reading' and `list').

The other aspect worth considering is that it was shown in Section \ref{sec.prod.res.vow.nurse.pil} (and discussed in \ref{prod.disc.nurse.age}) that young Scouser \is{identity}s have the most merged distributions and show the fewest style differences when \textsc{nurse} and \textsc{square} are analysed \emph{together}.
...
Young middle-class Liverpudlian \is{identity}s still have at least some sub-conscious \is{awareness} aware \is{\isi{awareness}}ness of fronter \textsc{nurse} variants as a typical feature of Scouse (which explains the fronting during perform \is{accent performance}ance), while young working-class speakers still hypercorrect \is{\isi{hypercorrection}} a bit in the most formal styles (which accounts for the fronting in the word list).
...
Among the oldest speakers there is hardly any conscious \is{awareness} aware \is{\isi{awareness}}ness of the feature (10\% of speakers comment on it, so only 1 in 10).
...
Data on style shifting (particularly when \textsc{square} realisations are also considered) and conscious \is{awareness} aware \is{\isi{awareness}}ness clearly show that the merger is not equally salient \is{salience} in the three age groups of speakers.
For the oldest speakers it is a \isi{marker}, aware \is{\isi{awareness}}ness of which is only just beginning.
In the middle generations, not only style shifting but also hypercorrect \is{\isi{hypercorrection}}ion is widespread.
Together with a steep increase in conscious \is{awareness} aware \is{\isi{awareness}}ness this shows that the feature is now, for many at least, a \isi{stereotype} that speakers actively (though rather unsuccessfully) try to avoid producing.
...
\section{Velar nasal plus: Indicator \is{indicator} with \isi{prestige} option}
\label{prod.disc.ng}
...
Compared to the oldest speakers in the sample, Scouser \is{identity}s aged between 19 and 29 still use velar nasal plus significantly more often, but if they are judged against the generation of their parents, they cannot be said to have more local realisations of this particular consonant as their rates are actually significantly lower.
...
From the old (where working class and middle class are not significantly different) to the middle age group speakers with higher social status seem to have taken up this variable (i.e. they seem to have become somewhat more aware \is{\isi{awareness}} of it) and increased their usage to a value which is then significantly higher than that of their working-class counterparts before subsequently lowering it again a bit so that the classes are, again, no longer statistically distinct \is{\isi{distinctness}} among younger Scouser \is{identity}s.

Not only social class, but also the gender of participant has an impact on how velar nasal plus is used.
The mixed linear effects regression revealed that female speakers have a higher \isi{PDF} (10.85\%), and thus more Scouse realisations, of /ŋ(g)/ than men (8.22\%).
...
Females have a comparatively high \isi{PDF} in the formal styles `word list' and `reading' as well as in `imitation \is{accent perform \is{accent performance}ance}', and only reduce this value somewhat in spontaneous speech.
Males, on the other hand, have comparatively low (and statistically identical) values for text reading, free speech, and \isi{accent perform \is{accent performance}ance} (although there is a slight rise from `free' to `imitation \is{accent perform \is{accent performance}ance}' which is close to significance), and only \isi{change} their pronunciation (in the same direction as women, i.e. towards more Scouse variants) when they are asked to read out a word list.
...
\begin{inparaenum}[(a)]
	\item they \isi{change} velar nasal plus pronunciations earlier (reading passage) on the way to the formal end of the style spectrum (whereas men need to reach the most formal register before there is any linguistic reaction), and
	\item they react more extremely at the other, most informal, end of the continuum (i.e. \isi{accent perform \is{accent performance}ance}), where the rise in \isi{PDF} is much less pronounced for male speakers.
\end{inparaenum}
Both points suggest that women are rather more aware \is{\isi{awareness}} of velar nasal plus than men.
...
From free speech /ŋ(g)/ pronunciations then become considerably more Scouse again when subjects are asked to perform \is{accent performance} a particularly strong Liverpool accent (\isi{PDF} is on about the same level as for text reading).
With one negligible exception, all these differences are statistically robust.

While the linear rise from spontaneous speech to text reading to the word list is evidence for some sort of at least subconscious \is{awareness} aware \is{\isi{awareness}}ness, this is not aware \is{\isi{awareness}}ness of velar nasal plus as a \emph{local feature of Liverpool English}, because in this case the slope would be wrong.
...
Some additional support for this interpretation can be found in \textcite[cf.][101]{newbrook1999}: In West \isi{Wirral} a not insignificant number of speakers endorsed realisations containing a plosive in both word-final \is{phonological context} and particularly in Intervocalic \is{\isi{phonological context}} position, probably because of ``sheer ignorance or confusion as to what the RP form might actually be''.
...
But of course the accent imitation \is{accent perform \is{accent performance}ance} task was still a highly artificial context and speakers presumably paid a lot of attention to their speech, albeit not in the traditional Labovian sense of the phrase.
All the same, getting the \isi{stereotype} `right' required them to focus on how they were articulating because this stereotypical \is{stereotype} accent was not their natural one (as is evidenced by the many comments about `falseness', cf. \ref{aware \is{\isi{awareness}}_res.eval}).
It is possible that the increased use of velar nasal plus during \isi{accent perform \is{accent performance}ance} is nothing but an artefact of a setting that required subjects to focus very intensely on their pronunciation.
I consider it more likely, however, that in addition to the spelling pronunciation aspect Liverpudlian \is{identity}s have at least some aware \is{\isi{awareness}}ness of velar nasal plus as a local feature as well.
...
Suffice it to say, for the moment, that whatever aware \is{\isi{awareness}}ness there is must definitely be subconscious \is{awareness}: Not a single subject in the extended secondary dataset (all 38 interviews) mentioned velar nasal plus as a typical feature of Liverpool English.
...
The style dimension, too, is particularly intriguing, and forbids the classification of /ŋ(g)/ as an \isi{indicator}, since there are clear differences between registers.
...
In light of consistent, if somewhat difficult to interpret style-shifting, I conclude, then, that velar nasal plus is a \isi{marker} for all three age groups investigated and that younger speakers do neither provide evidence for changing \isi{salience} of the feature nor do they, in fact, use the local variant more than their parents' generation.

\section{Lenition: From \isi{indicator} to \isi{stereotype}}
\label{prod.disc.k}
...
Scouser \is{identity}s aged between 56 and 85 have mean \isi{PDF} values comparable to speakers who are between 30 and 55 years old, so from a statistical point of view, and in this particular context, the two groups can actually be considered as one.
Younger Liverpudlian \is{identity}s exhibit a significantly higher mean \isi{PDF} than both speakers of their parents' or grandparents' generation.
The apparent contradiction between the raw data and the mixed-effects regression has been shown to be mostly due to \emph{like} (as a discourse \isi{marker} and quotative particle), because, among the youngest speakers, this word is both considerably more frequent and also realised with a higher average \isi{PDF} than in the other two groups. 
...
After all, the fact that young Liverpudlian \is{identity}s frequently say [laɪç] (or any other words that they are more likely to realise with a fricative than older speakers) probably does contribute considerably to many laypersons' impression that Scouse is getting stronger since lenition is not only one of the best known but also most stigmatise \is{stigmatisation}d features (cf. \ref{prod.disc.k.aware \is{\isi{awareness}}}).
Interestingly, the same differences (non-significant between old and middle, but significant between middle and young group) also surface when observations pertaining to \emph{like} are removed from the dataset altogether, so \emph{like} is clearly not the only factor, and young Liverpudlian \is{identity}s really do seem to be Scouser \is{identity} than those of the middle-aged and old group.
...
From the middle-aged to the young speakers there is then only a slight (and non-significant) further increase of \isi{PDF}, so that young female Scouser \is{identity}s do not use lenition more than their parents' generation already did.
...
It should be noted that the differences are subtle, though: /k/ realisations of women and men are statistically distinct \is{\isi{distinctness}} in all three generations of speakers.
...
The fact that women are more Scouse than men in the middle group might therefore suggest that the variable has lost \isi{salience} in this generation, but additional evidence refutes this hypothesis (see \ref{prod.disc.k.aware \is{\isi{awareness}}}).
...
The mixed linear effects regression showed that, all other things being equal, middle-class subjects have lower \isi{PDF} rates and thus more standard realisations of /k/ than working-class Liverpudlian \is{identity}s, which is the expected outcome for a salient \is{salience} variable.
Just as with gender, the class difference is highly significant in all speaking styles except accent imitation \is{accent perform \is{accent performance}ance} (where \isi{PDF} means are again highest), which shows that the strong association of /k/ lenition with a stereotypical \is{stereotype} Scouse accent is not only shared among Liverpudlian \is{identity}s of both genders, but also across different socioeconomic classes.
...
For the oldest speakers, there is actually \emph{no} significant difference in the use of lenition between working-class and middle-class Liverpudlian \is{identity}s.
...
This final point indicates that the \isi{salience} of this variable has increased among middle-class speakers: they are more aware \is{\isi{awareness}} of lenition (and its non-standardness) and therefore try to avoid it.
To explain the opposite trend in the working class one could assume that \isi{salience} in this group has simultaneously decreased, but this is not what is happening (cf. \ref{prod.disc.k.aware \is{\isi{awareness}}}).
Rather, /k/ lenition must have acquired covert \isi{prestige} as a \isi{marker} of local \isi{identity}.
For Scouser \is{identity}s of lower socioeconomic classes this covert \isi{prestige} seems to be more important than the social stigma attached to it, whereas for middle-class speakers priorities are reversed.
...
Only when it comes to accent imitation \is{accent perform \is{accent performance}ance} is there a steep rise of \isi{PDF} towards more fricative-like local variants.
These two groups of speakers thus have, at best, a two-way style distinct \is{\isi{distinctness}}ion (stereotypical \is{stereotype} accent vs. everything else), so their aware \is{\isi{awareness}}ness of the variable, while not nonexistent, seems to be limited.
Young Scouser \is{identity}s, however, presents a textbook case of style shifting as we would expect it for a socially meaningful variable: There is a steady, statistically significant, and almost perfectly linear increase of \isi{PDF} from the most formal to the least formal register.
Compared to the other two groups, the youngest speakers in my sample manipulate lenition in a much more fine-grained way, which shows that aware \is{\isi{awareness}}ness has reached a level in this group that is considerably higher than for older Liverpudlian \is{identity}s.

It has already been pointed out above that women show more aware \is{\isi{awareness}}ness of lenition than men do.
This higher degree of sensitivity also shows in (slightly) different style shifting patterns.
Female speakers exhibit more systematic and more pronounced style differences than those that are observed for male subjects, so they are not only more sensitive to /k/ lenition in general (which would just translate to lower absolute \isi{PDF} values, but not necessarily different style shifting patterns).
With respect to lenited variants of /k/ women also are more susceptible to the style dimension.
An additional relevant point here is that when the data are split up along the gender dimension, the youngest speakers are still the ones that show the most systematic style shifting pattern, and this is true for both women \emph{and} men, which shows that the increase in aware \is{\isi{awareness}}ness along the age dimension is not limited to just one of the two genders, but is really primarily a question of age.
The fact that style shifting patterns are least different in the young group is further evidence for the idea that aware \is{\isi{awareness}}ness of lenition is more universal in this generation than in the other two.
...
This looks very much like the hypercorrect \is{\isi{hypercorrection}}ion that was observed for \textsc{nurse} (cf. \ref{prod.disc.ng.style}), but it is difficult to see why it would affect these two sub-groups in particular.
...
Such a result is unexpected because it could be taken to imply that middle-class speakers pay less attention than their working-class counterparts to how often they use stigmatise \is{stigmatisation}d variants in a particular register.
I believe, however, that this is not the case.
It has to be taken into account that the mean \isi{PDF} values of middle-class speakers, especially in the middle and the young generation, are relatively low across the board (accent imitation \is{accent perform \is{accent performance}ance} excepted), and considerably lower than even the most standard-like values measured for the working class.
I believe that middle-class Liverpudlian \is{identity}s just try to avoid lenited /k/ variants altogether, even in spontaneous speech, and that they just cannot get much more standard than they already are, even if they wanted to.
...
\label{prod.disc.k.aware \is{\isi{awareness}}}

Lenition of /k/ attracts, by far, the largest number of explicit comments and evaluations in my sample, which is clear evidence for the fact that it is considerably more salient \is{salience} than the other three variables investigated in this thesis.
...
In the middle-aged group this rate has already risen considerably to around 70\%, and when it comes to the youngest Scouser \is{identity}s in the sample, each and every one of them expresses conscious \is{awareness} aware \is{\isi{awareness}}ness of lenition in velar plosives, so the variable has reached full and universal \isi{stereotype} status.
...
It is not quite clear \emph{why} aware \is{\isi{awareness}}ness has increased in the way that it has.
...
Furthermore, a separate study would first have to investigate whether perform \is{accent performance}ances in the media and on stage really \emph{have} \isi{change}d in the last decades as to which features they focus on.
...
We could also argue that increasing aware \is{\isi{awareness}}ness in younger speakers is based on the changing usage of the variable.
For instance, people's aware \is{\isi{awareness}}ness of non-standard variants might go up when they realise that middle-class speakers increasingly start to avoid them.
However, this would lead to the chicken and egg problem commonly encountered in \isi{salience} research (cf. \ref{sec.sal.sal.circle}): if aware \is{\isi{awareness}}ness goes up because usage \isi{change}s, then what triggers \isi{change} in usage in the first place?
Whatever the reason for the increase in conscious \is{awareness} aware \is{\isi{awareness}}ness is, explicit comments clearly show that lenition of /k/ is even more of a \isi{stereotype} for younger Liverpudlian \is{identity}s than it is for the older generations.

Why, then, do younger Liverpudlian \is{identity}s use this feature more than their parents or grandparents despite the fact that they know about it?
...
Lenition of velar plosives is strongly associated with these pronounced Liverpool accents and considered one of its most distinct \is{\isi{distinctness}}ive features.
Across all three generations, subjects judge it rather negatively (`annoying', `makes you unintelligible \is{unintelligibility}'), possibly because at least some of them have personal experience of outsiders using this variable to make fun of Liverpudlian \is{identity}s.
Higher proportions of lenited variants among younger Scouser \is{identity}s can thus \emph{not} be explained by a different (overt) attitude towards the feature.

Another explanation could be that young speakers just cannot help using lenition.
The regular style-shifting that was found, however, rather suggests that particularly younger speakers can at least sub-conscious \is{awareness}ly control their usage of the local variant quite well.
It is possible that some Scouser \is{identity}s are \emph{generally} aware \is{\isi{awareness}} of the variable but not, for some reason, of its presence \emph{in their own speech}.
...
Just as outlined above for the class differences (cf. \ref{prod.disc.k.social}), I would therefore argue that the higher use of lenition among the younger speakers is primarily due to the covert \isi{prestige} that lenited variants appear to have acquired.
...
Inter-group differences in style shifting suggest that for the old and the middle-aged speakers (sub-conscious \is{awareness}) aware \is{\isi{awareness}}ness is lower than in the young group because the former show only limited style aware \is{\isi{awareness}}ness, while the latter present a textbook case of Labovian style shifting, with non-standard variants getting consistently and linearly more likely the more informal the communicative context.

Data on explicit comments and judgements further showed that conscious \is{awareness} aware \is{\isi{awareness}}ness increases in a linear fashion from the oldest to the youngest Scouser \is{identity}s.
My data thus suggest that lenition of /k/ has developed from a (beginning) \isi{marker} in the old group (where only a minority is conscious \is{awareness}ly aware \is{\isi{awareness}} of the variable), to a consolidated \isi{marker} (for the minority) or \isi{stereotype} (for about three out of four speakers) in the middle generation, and then finally to a fully-fledged and universal \isi{stereotype} in the young group of speakers, where not only \emph{every} speaker knows about the feature but where style shifting is also most consistent and regular.
Liverpudlian \is{identity}s aged 29 and younger are therefore not only `more Scouse' than their parents and grandparents with respect to this variable, but they are also the ones that are most aware \is{\isi{awareness}} of this feature of Liverpool English.

\section{Summary}
\label{prod.disc.summary}

People of all age groups closely link Scouse to their local \isi{identity}.
Generally speaking, they are both aware \is{\isi{awareness}} and proud of its distinct \is{\isi{distinctness}}ness as an accent, although some subjects also see this as something that can be problematic, because they know about the negative connotations that a Scouse accent can carry, particularly for outsiders.
To a degree, some of these external \isi{stereotype}s (and their evaluation!) seem to have been internalised, as when Liverpudlian \is{identity}s base their list of typical features of their own accent on stereotypical \is{stereotype} perform \is{accent performance}ances by outsiders.
On the whole, inside evaluations are often ambivalent: `Light' accents are seen as adding an acceptable amount of local flavour and carrying positive connotations such as down-to-earthness, while extremely `strong' Scouse accents receive much less favourable judgements (this aspect is also reported in \citealt[33]{delyon1981}, often because they appear as exaggerated, false, and inauthentic \is{authenticity} \is{\isi{authenticity}}.
Old and middle-aged Liverpudlian \is{identity}s believe these `exaggerated' accents to be more common among younger speakers, but this verdict is not shared by young Scouser \is{identity}s who appear to reject `plastic' accents just as much as the older subjects (interestingly, one subject interviewed by \citeauthor{delyon1981} in 1979 claimed already that some Liverpudlian \is{identity}s deliberately `exaggerated' the accent, \citealt[cf.][30]{delyon1981}).
...
All the same, there is some evidence that middle-aged speakers seem to be particularly sensitive to the negative \isi{image} of Scouse, which shows in the hypercorrect \is{\isi{hypercorrection}}ion that these speakers exhibit for the two salient \is{salience} variables \textsc{nurse} and /k/.
...
They know about the negative \isi{image} of their city and their accent, but at least to a certain degree they consider these attitudes to be outdated and unjustified.
...
However, young Scouser \is{identity}s are not more Scouse in every respect.
...
\begin{inparaenum}[(1)]
	...
	\item /k/ lenition, finally, is the variable which not only generates the most significant differences in usage among social groups and the most systematic style shifting patterns, but it is also the one that most subjects conscious \is{awareness}ly know about (in every generation).
	As the feature that is clearly a \isi{stereotype} and even a \isi{shibboleth} for many, or even most, Liverpudlian \is{identity}s it is without a doubt the most socially salient \is{salience} variable investigated in the context of this thesis.
\end{inparaenum}