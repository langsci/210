...
Instead, \textcite[237]{watson2007a} found Scouse to be ``getting Scouser \is{identity}'', at least with respect to some variables.
...
This approach provides the opportunity to address several additional questions along the way, as it were, such as whether younger Liverpudlian \is{identity}s have stronger local accents than older speakers in \emph{every} respect, or how these \isi{change}s are related to local \isi{identity}, the internal as well as external \isi{image} of their city, and attitudes of speakers towards their variety.
...
As far as I am aware \is{\isi{awareness}}, for instance, there is still no complete descriptive account of Liverpool English except \cite{knowles1973}, which is now quite dated and also clearly and explicitly \emph{not} a truly variationist study of the kind \textcite{watson2007} refers to.
...
An a priori limitation \is{accent perform \is{accent performance}ance} of my thesis is that it is only concerned with Scouse as an accent.
...
Social differentiations of subjects (for the production data) are therefore comparatively coarse, and the size of the speaker sample does not permit much more fine-grained distinct \is{\isi{distinctness}}ions.
...
Despite the fact that \emph{\isi{salience}} is part of the sub-title of this work and notwithstanding that the term will turn up again and again in what is to follow, the present study is \emph{not} a book \emph{about} \isi{salience} per se (cf. Chapter \ref{ch.sal}).
...
Essentially, social \isi{salience} will be the scale used to measure the degree of aware \is{\isi{awareness}}ness of, and attention paid to, a particular variable.
I will then show that the level of aware \is{\isi{awareness}}ness correlates with the strength of the \isi{priming} effect.
How and why aware \is{\isi{awareness}}ness came about in the first place is irrelevant for this purpose and will not be discussed any further.

\section{Structure of the book}
\label{sec.intro.structure}

Chapter \ref{ch.hist} sketches the history of the city of Liverpool and its accent to give the reader an idea about the social \isi{change}s that have taken place in this city and how they might influence the attitudes of speakers from different generations towards Scouse and questions of local \isi{identity}.
...
Chapters \ref{ch.prod_results_vow} (vowels) and \ref{prod.res.con} (consonants) contain the quantitative analysis of the data gathered from the sociolinguistic interviews, while Chapter \ref{prod.res.qual} presents a recapitulatory qualitative analysis of participants' explicit comments about (specific features of) their accent, local \isi{identity}, and the like.
