\chapter{A few words on \isi{salience} and \isi{exemplar} theory}
\label{ch.sal}
...
Both concepts are of prime \is{\isi{priming}} importance for this study, and it is therefore vital that some basic assumptions pertaining to these notions be defined before we move on to the empirical results that they will help interpret and explain.

	\section{Salience \is{salience}}
	\label{sec.sal.sal}
...
However, this account will deliberately be as brief as possible; more detailed analyses of \isi{salience}, its history, and use in sociolinguistics can, for example, be found in \cite{kerswillwilliams2002}, \cite{racz2013}, and \cite{auer2014} --- all three of which are also the primary sources of what is to follow below.
		...
		\subsection{Salience \is{salience} and circular \is{\isi{circularity}}ity}
...
As the \emph{Oxford English Dictionary} puts it, \isi{salience} (in psychology) is the ``quality or fact of being more prominent in a person's aware \is{\isi{awareness}}ness or in his memory \is{memory structure} of past experience'' --- in simpler terms, \isi{salience} is the quality of `sticking out' from the rest.
\textcite[81]{kerswillwilliams2002} stay very close to this general description when they define (socio-)linguistic \isi{salience} as ``a property of a linguistic item or feature that makes it in some way perceptually and cognitively prominent''.
...
This issue is addressed by \textcite[82]{kerswillwilliams2002} as well, who argue that when \isi{salience} is used as ``a potential explanatory factor, (\ldots) the concept all too easily lapses into circular \is{\isi{circularity}}ity and mere labelling'', a point that is illustrated very well by their critique of \textcite{trudgill1986}.
...
The problem is that \isi{stigmatisation} and the \isi{change} that it often entails (for example, when people start avoiding the stigmatise \is{stigmatisation}d variant) are not only the prerequisites of \isi{marker} status, but also its outcome --- people are aware \is{\isi{awareness}} of non-standard variants because they are stigmatise \is{stigmatisation}d, and the variants are stigmatise \is{stigmatisation}d because people are aware \is{\isi{awareness}} of them.
...
\end{inparaenum}
To avoid this kind of circular \is{\isi{circularity}}ity it is therefore absolutely crucial to establish the \isi{salience} status of the test variables \emph{independently}, which is why the production data were collected.

Research based on the notion of \isi{salience} is perhaps particularly prone to falling victim to the circular \is{\isi{circularity}}ity trap because ``\isi{salience} \emph{attempts} to combine both structural (language-internal) factors with sociolinguistic and psychological (extra-linguistic) factors in a single explanatory concept'' \parencite[83, my emphasis]{kerswillwilliams2002}, but many researchers actually focus primarily on one particular aspect only.
However, if \isi{salience} is to have any explanatory value (which necessitates avoiding circular \is{\isi{circularity}}ity), ``it \emph{must} have recourse to extra-linguistic factors, which will be a combination of cognitive, social psychological or pragmatic factors'' \parencite[83, my emphasis]{kerswillwilliams2002}.

		\subsection{Cognitive vs. social \isi{salience}}
...
Moreover, \enquote{from a sociolinguistic perspective, the choice of features which become [sociolinguistically] salient \is{salience} is in large part an arbitrary one} and seems to depend primarily on \enquote{community consensus} \parencite[56]{llamasetal2017}, which is why I would argue that, \emph{for a sociolinguist}, the question of what makes something cognitively salient \is{salience} can be considered secondary to the (descriptive) knowledge about which features the community agreed to pay attention to.
		...
		\subsection{Salience \is{salience} in this study}
...
While Chapter \ref{ch.var} provides a rough distinct \is{\isi{distinctness}}ion into salient \is{salience} and non-salient \is{salience} variables of Liverpool English as they are presented in the literature, these classifications are
\begin{inparaenum}[(1)]
	\item primarily based on the observations of experts (\isi{dialect}ologists) or laypersons with a special interest in linguistic phenomena (e.g. the authors of the \emph{Lern Yerself Scouse} series), and/or
...
This is because ``language users are usually very much aware \is{\isi{awareness}} of particular words or \isi{intonation} patterns \emph{other} people use (\ldots), but are much less attentive to phonetic differences'' \parencite[3, emphasis in the original]{racz2013}.
...
In this study, as in many others, this indirect measure is based on the \emph{effects} of social \isi{salience}, the most important of which include \emph{\isi{social stratification}}, \emph{hypercorrect \is{\isi{hypercorrection}}ion}, and, above all, \emph{style shifting}.
...
As a general term, hypercorrect \is{\isi{hypercorrection}}ion refers to the ``misapplication of an imperfectly learned rule'' \parencite[126]{labov1972}.
...
In the present study, the term hypercorrect \is{\isi{hypercorrection}}ion will mostly be used in the more general sense, which extends its scope to any case where a given rule has been learned `imperfectly', e.g. when speakers use an even more non-standard variant in more formal speech styles (compared to spontaneous speech) or when they correct the `wrong' member of a merger.
Both applications of hypercorrect \is{\isi{hypercorrection}}ion imply (sub-)conscious \is{awareness} aware \is{\isi{awareness}}ness of socially meaningful variation, as both the target (in the Labovian definition) or the rule (in the more general reading) have a social component.
...
In consequence, ``social aware \is{\isi{awareness}}ness of a given variable corresponds to the slope of style shifting'' \parencite[196]{labov2001a}.

Based on \isi{social stratification}, hypercorrect \is{\isi{hypercorrection}}ion, and style shifting, \citeauthor{labov1972}'s \citeyear{labov1972} hierarchy of \emph{\isi{indicator}s}, \emph{\isi{marker}s}, and \emph{\isi{stereotype}s} is a convenient way of categorising linguistic variables according to their sociolinguistic \isi{salience}.
...
The \isi{speech community} itself is, however, completely unaware \is{awareness} \is{\isi{awareness}} of the feature and uses it to the same degree in all communicative situations, so there is no style shifting.
When a \isi{speech community} starts to become (sub-conscious \is{awareness}ly) aware \is{\isi{awareness}} of a feature it is increasingly invested with social meaning and associated with a particular degree of (non-)formality.
...
A \emph{\isi{stereotype}} finally, does not only exhibit \isi{social stratification} and style shifting, but has actually crossed the threshold to conscious \is{awareness} aware \is{\isi{awareness}}ness, and is explicitly commented on by members of the \isi{speech community} \parencite[cf.][178--180]{labov1972}.
Speakers are thus completely unaware \is{awareness} \is{\isi{awareness}} of \emph{\isi{indicator}s}, only sub-conscious \is{awareness}ly aware \is{\isi{awareness}} of \emph{\isi{marker}s}, and fully conscious \is{awareness} of \emph{\isi{stereotype}s}.
...
In any case, this question does not affect the usefulness of the \isi{indicator}-\isi{marker}-\isi{stereotype} hierarchy as a means of categorising variables according to how aware \is{\isi{awareness}} speakers are of them.
...
There is a large degree of overlap between these terms and Labov's \isi{indicator}-\isi{marker}-\isi{stereotype} distinct \is{\isi{distinctness}}ion, while, of course, the two frameworks are not completely identical.
...
As we will see later, features of Scouse that can be classified as Labovian \isi{stereotype}s \emph{are} actually used in \isi{accent perform \is{accent performance}ance}s, and do not seem to be disappearing either, so it might seem preferable to use \citeauthor{johnstoneetal2006}'s terminology.
...
Since the degree of sociolinguistic aware \is{\isi{awareness}}ness is what this study is interested in, I will therefore stick to the more traditional Labovian terminology.

\textcite[6]{racz2013} criticises the \isi{indicator}/\isi{marker} distinct \is{\isi{distinctness}}ion as ``impl[ying] a complete absence of gradience'' while linguistic aware \is{\isi{awareness}}ness should be conceived of as having ``many levels, very few categorical''.
...
It seems to me that it is quite possible to distinguish, for example, different \emph{degrees} of style shifting (How many styles are kept distinct \is{\isi{distinctness}}? How significant are the differences?), \emph{in addition to} a simple binary assessment of whether style shifting is present or not, and the same should hold for \isi{social stratification} or hypercorrect \is{\isi{hypercorrection}}ion.
...
No matter how fine-grained the classification, however, what I intend to do is clearly what \textcite{kerswillwilliams2002} have called using \emph{\isi{salience}} as a label, which means that it is ``no more than another term for the \isi{indicator}/\isi{marker} distinct \is{\isi{distinctness}}ion'' \parencite[32]{racz2013}.
...
Salience \is{salience}, in this dissertation, will thus be understood as meaning the amount of (social) aware \is{\isi{awareness}}ness speakers have of a sociolinguistic variable.
As such, it will be measured by the presence and, if applicable, degree of \isi{social stratification}, hypercorrect \is{\isi{hypercorrection}}ion, style shifting, and explicit comments and evaluations.

	\section{Exemplar theory}
	\label{sec.sal.\isi{exemplar}}
...
The reader is referred to \citealt{pierrehumbert2006} for a more detailed discussion.
		...
		\label{sec.sal.\isi{exemplar}.basic}
...
It will become clear in the following paragraphs that activation of \isi{exemplar}s via indexed information is a crucial aspect of \isi{exemplar} theory for any sociolinguistic \isi{priming} study.
  ...
  \label{sec.sal.\isi{exemplar}.socio}
...
Phonemes do exist as mental categories, but as just explained for episodic approaches more generally, they have to be viewed as ``clusters of similar experiences'', whose centres of gravity are malleable and can be \isi{change}d by ``incremental updating'' of remembered \isi{exemplar}s \parencite[cf.][519]{pierrehumbert2006}.
...
And of course any \isi{exemplar} can be indexed with information that is somehow related to the wider context the experience was made in (cf. \ref{sec.sal.\isi{exemplar}.basic}).
...
These two fricatives are primarily distinguished by their central \isi{frequency}, and the boundary between the two phonemes (i.e. the point where, perceptually, a /ʃ/ becomes a /s/) is typically lower for male than for female realisations.
...
Of course, the sex/gender distinct \is{\isi{distinctness}}ion is a rather crucial one in language perception, as men have vocal tracts that are physiologically different from those of women, which results in markedly lower resonance frequencies for the former.
...
These labels had a significant effect: although everyone received the same acoustic input, subjects who had been prime \is{\isi{priming}}d for `Canada' were significantly more likely to perceive Canadian Raising than those who had been prime \is{\isi{priming}}d for `Michigan' \parencite[cf.][64--68]{niedzielski1999}.
...
If the prime \is{\isi{priming}} is `Michigan', however, perceivers activate \isi{exemplar}s that are indexed with `US standard English' (because Detroiters consider themselves to be speakers of standard English).
...
They had an essentially identical methodology, but used the New Zealand-Australia opposition to prime \is{\isi{priming}} participants, instead of Michigan-Canada as in \citeauthor{niedzielski1999}'s study.
...
Results were comparable to \citealt{niedzielski1999}: subjects prime \is{\isi{priming}}d for New Zealand were more likely to perceive centralised tokens, while subjects prime \is{\isi{priming}}d for Australia were more likely to report more Australian percepts \parencite[cf.][359--363]{hayetal2006a}.
...
Instead of an explicit label on an answer sheet they used stuffed toys commonly associated with Australia (kangaroo, koala) and New Zealand (kiwi) to prime \is{\isi{priming}} perceivers.
...
The reason for this is that long-term memory \is{memory structure} is not a mirror \isi{image} of ``undifferentiated raw experience'' \parencite[525]{pierrehumbert2006}.
...
Sociolinguistic \isi{salience} will be understood as a scale of (sub-)conscious \is{awareness} aware \is{\isi{awareness}}ness.
Features will be classified with respect to Labov's \isi{indicator}-\isi{marker}-\isi{stereotype} hierarchy which, in turn, will be based on the presence and extent of \isi{social stratification}, style shifting, and hypercorrect \is{\isi{hypercorrection}}ion.
...
It is therefore to be expected that \isi{exemplar} \isi{priming} effects either do not occur at all or are at least considerably weaker when the test variable does not enjoy a high degree of (conscious \is{awareness}) aware \is{\isi{awareness}}ness among perceivers.