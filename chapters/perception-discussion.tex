...
They hypothesise (probably rightly so) that New Zealanders from higher social classes are ``more able to travel to Australia'' and therefore have ``more stored \isi{exemplar}s indexed with `Australian'~'' which can be activated by a prime\is{priming}.
...
As an alternative explanation, I would like to argue once more for aware\is{awareness}ness, or, in the terminology used above, attention.
...
On the basis of how \textcite{haydrager2010} explained their social class effect, it would have been expected that people who are less familiar with Scouse show less of a \isi{priming} effect because they have fewer \isi{exemplar}s that can be activated by the prime\is{priming}.
...
Subjects could then be said to hyper-correct their perception more for word-final\is{phonological context} stimuli because the distance between the activated \isi{exemplar}s and the actual acoustic input is greater in this context due to stronger activation by the prime\is{priming}.
...
This analysis, like any piece of research, comes with a number of short-comings and limitation\is{accent performance}s, a few of which will be addressed in the following paragraphs.
...
When men, on the other hand, are prime\is{priming}d for `Australia' they not only activate their Australian \isi{exemplar}s but also their negative associations with that country.
...
Despite some \isi{linguistic insecurity}, overt comments about Liverpool and its accent essentially tell the same story (cf. \ref{aware_res.eval}).
...
When the prime\is{priming} is a ``moderate category'' and the stimulus is ``ambiguous'', then the stimulus ``should in fact be judged as an instance of that category''.
If, on the other hand, the category used for \isi{priming} is ``extreme'', then the ``ambiguous target should not be categorized within the prime\is{priming}d category'' because there is little or even no match between the stimulus and the prime\is{priming}.
...
The prime\is{priming}s fell into one of four categories: ``extremely nonhostile'' (e.g. \emph{Pope John Paul} or \emph{Santa Claus}), ``moderately nonhostile'' (\emph{Robin Hood}, \emph{Henry Kissinger}), ``moderately hostile'' (\emph{Alice Cooper}, \emph{Menachem Begin}), and ``extremely hostile'' (\emph{Dracula}, \emph{Adolf Hitler}).
...
Results of this experiment were pretty clear and as expected: There was an interaction of prime\is{priming} and its ``extremeness''.
Participants prime\is{priming}d with moderately non-hostile \emph{or} extremely hostile \isi{exemplar}s rated ``Donald'' as less hostile, and more friendly and kind than did subjects who had been exposed to the moderately hostile \emph{or} extremely non-hostile category \parencite[cf.][1109]{herr1986}.
In other words, subjects rated the fictitious person as similar if the prime\is{priming} was moderate, but reversed the effect when \isi{priming} used ``extreme'' categories; compared to, say, Adolf Hitler, Donald actually seems to be a pretty nice person.
...
The only assumption that needs to be accepted to explain the direction of \isi{priming} as a \isi{contrast effect} is that the prime\is{priming} used in this study was what \citeauthor{herr1986} calls ``extreme'', and this does not seem too far-fetched.
...
The authors argue that women behaved as expected because their associations with Australia are positive or neutral, whereas men have more negative attitudes due to the sporting rivalry and wish to disassociate\is{disassociation} from the invoked concept `Australia'.
It is equally possible, however, that the gender differences that were found in these studies are ultimately caused by different levels of aware\is{awareness}ness of variation in the test variable, rather than different attitudes.
...
For men, on the other hand, the distinct\is{distinctness}ion is very (!) salient\is{salience}.
If their categories are (felt to be) more distinct\is{distinctness} or distant from one another, then the prime\is{priming} `Australia' would actually be much more extreme for them than for the women.
Possibly just as extreme as the prime\is{priming} `Liverpool' was for the British subjects in my study, which might explain why both my participants and the male perceivers in New Zealand show a very similar \isi{contrast effect}.
