...
Whatever the precise details of its evolution, in Liverpool developed what Trudgill calls ``an accent rather more `modern' than that of its hinterland'' \citep[70]{trudgill1999} and that he describes as being ``well known to most British people, and very distinct \is{\isi{distinctness}}ive''.
For instance, \textcite{montgomery2007} found `Scouse' to be the \isi{dialect} area most often delimited and labelled by lay participants in a map drawing task.
Scouse also turned out to be the most stigmatise \is{stigmatisation}d of the language varieties mentioned by said participants (cf. \cite[194 and 254]{montgomery2007}).
Furthermore, participants provided more linguistic characteristics for Scouse than for any other \isi{dialect} area, indicating that Scouse (along with Geordie) has a higher cultural \isi{salience} than most other varieties in England.
...
\textcite[15]{crowley2012} also emphasises the \isi{salience} of Scouse when he writes that ``(\dots) in Britain and Ireland at least, Liverpool and Liverpudlian \is{identity}s are most widely recognized by their association with a distinct \is{\isi{distinctness}} form of spoken language``.
...
\citet[18]{wales2006} writes that \isi{Merseyside} is a ```transition' [zone] between Northern and Midland \isi{dialect} speech'' and \citet[72]{trudgill1999} claims that Scouse is in some respects as southern as it is northern.
Much of its distinct \is{\isi{distinctness}}iveness is due to phonetic rather than phonemic divergence from the surrounding varieties. \textcite{knowles1973} describes Scouse as being phonologically North(west)ern but phonetically Anglo Irish (\citealt[cf. also][80]{knowles1978}, but see Section \ref{sec.hist.19} concerning Irish dominance in the \isi{dialect} mix).

The only comprehensive description of Scouse as a whole so far is \citet{knowles1973}, which is based on interview data from two Liverpool electoral wards --- Aigburth to the south and Vauxhall to the north of Liverpool city centre.
...
That said, Knowles is aware \is{\isi{awareness}} of some of these shortcomings, calling his description of the Scouse vowel system ``admittedly speculative'' and ``put forward extremely tentatively'' \citep[111]{knowles1973}.
...
\citet{knowles1973} talks at length about Scouse \isi{intonation} and indeed it is a feature which rather quickly strikes the outsider when first talking to a Liverpudlian \is{identity}.
...
\citeauthor{wales2006} remarks that although ``[supra-segmentals] are such readily distinct \is{\isi{distinctness}}ive \isi{marker}s of regional origin (\ldots) they have been quite seriously under-researched'' \citeyearpar[201]{wales2006}.
...
Just as for the segments, he claims that Liverpool \isi{intonation} is, in general, ``phonologically North-Western English, but largely phonetically Anglo-Irish'' \citep[225]{knowles1973} --- a claim that has to be based on a `phonology of \isi{intonation}', which indeed he sketches in his thesis.
...
Just like the Lancashire \isi{dialect}s it is derived from, Scouse was still rhotic in the 19\textsuperscript{th} century, but it has now lost all traces of this rhoticism \citep[cf.][149]{knowles1997} and is just like RP in this respect.
\emph{Pre}-vocalic /r/ is often realised as a flap in broad Scouse --- especially in Intervocalic \is{\isi{phonological context}} position, but also in onset clusters (cf. \citealt[107 and 329--330]{knowles1973}; \citealt[352]{watson2007}).
...
Realisations containing a velar plosive occurred frequently, both in Intervocalic \is{\isi{phonological context}} and in word-final \is{phonological context} contexts.
The majority of speakers did not exhibit any style shifting with this variable (although \isi{marker} patterning did occur for some of them), which ``suggests limited \isi{salience}'' of this variable in the wider Liverpool region \parencite[98]{newbrook1999}.
...
At least as far as the alveolar plosives are concerned the three `cardinal' categories nevertheless remain distinct \is{\isi{distinctness}} \parencite[cf.][327 and 252--253]{knowles1973}.

Based on his \citeyear{knowles1973} data, \citeauthor{knowles1973} found that the majority of Liverpudlian \is{identity}s used ``stops with incomplete closure'' at least every now and then and many apparently even realised lenited stops in rather formal speaking styles.
...
Especially in Intervocalic \is{\isi{phonological context}} environments lenition is phonetically motivated, which is the reason why it occurs frequently in this context, both in typological terms and in Liverpool English in particular \parencite[cf.][230 and 243]{honeybone2001}.
...
``The full patterning of Liverpool lenition'', however, constitutes ``a creative act'', perform \is{accent performance}ed by ``the young generations of young Liverpudlian \is{identity}s who were forming or focusing the koine'' \parencite[132]{honeybone2007}.
...
\textcite{honeybonewatson2013} hypothesise that ``speakers are not very clearly aware \is{\isi{awareness}} of the existence of the phenomenon'' because it is (a) a comparatively recent, and (b) a sub-phonemic feature which does not entail the collapse of categories \parencite[cf.][329--331]{honeybonewatson2013}.
...
Many middle-class speakers, however, tend to keep the two sets distinct \is{\isi{distinctness}}.
...
Some confusion as to which vowel should be used in which words exists, and hypercorrect \is{\isi{hypercorrection}}ions and mistakes occur (\cite[286--287]{knowles1973} and \cite[83]{knowles1978}).
...
It is also mostly this age group that makes use of [u:] as a typical accent feature in the imitation \is{accent perform \is{accent performance}ance} task (see Section \ref{sec.prod_method.interview}).
...
As another ``\isi{Merseyside} feature'' \citet[72]{trudgill1999} notes the \textsc{nurse}-\textsc{square} merger\footnote{Patrick Honeybone (p.c.) is critical of calling this feature a merger because the term either implies \enquote{speakers are actively/synchronically abandoning a contrast, or at least that this is a merger which has happened in the history of Scouse [as opposed to before the formation as a new \isi{dialect}]}, neither of which he considers to be true. I tend to agree, but, for reasons of convenience, have decided to follow other studies \parencite{trudgill1999,watsonclark2013} in using the label \enquote*{merger} nonetheless.}, i.e. the fact that words such as \emph{fair} and \emph{fur}, or \emph{purr} and \emph{pair} can be (near-) homophones in Liverpool English.
...
For example --- although this is not very exciting news --- he states that, generally, the working-class residents of Vauxhall do not make this distinct \is{\isi{distinctness}}ion, whereas the middle-class speakers from Aigburth usually do, with the Aigburth women topping the list (which is, this time, in line with most research on gender differences that followed).
The degree of difference between the two vowels can, however, be very subtle, to the point that ``a gesture towards the \isi{prestige} standard'' (for the speaker), ``may be for the hearer just another variant of a \isi{dialect} vowel'' \citep[cf.][295--297]{knowles1973}.
...
Concerning possible sources of this merger, \citet[128]{honeybone2007} mentions the \isi{dialect} of South Lancashire as the most obvious candidate.
...
What these reports do show, however, is that ``the pre-\emph{r} vowels in these words are susceptible to considerable variation in Hiberno-English varieties (\ldots) in ways which would have differed from those supplied by Welsh, Scottish and most English \isi{dialect}s during koineisation'', which is why Irish influence does seem plausible \citep[128]{honeybone2007}.
...
Rather, it was actively selected from ``the mix of \isi{dialect} contact'', adapted, \isi{change}d, and made a part of Liverpool English \parencite[cf.][129]{honeybone2007}.
...
This received, and comparatively broad, distinct \is{\isi{distinctness}}ion into salient \is{salience} and non-salient \is{salience} variables constitutes the starting point and the basis of the present study, and will be updated and refined in the following chapters.